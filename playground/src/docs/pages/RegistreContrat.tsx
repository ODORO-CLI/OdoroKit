    ...
    <div ref={article}>
      ...
      */}
      ...
      <PageHeader
        ...
        lead="Cinq façons d'adapter un composant sans en éditer la source — et donc sans avoir a reporter la retouche a chaque mise a jour."
      />
      ...
      <Callout>
        ...
        haut de la fenêtre, et la pastille en bas. Faites defiler.
      </Callout>

      <Section
        title="L’échelle"
        lead="Les niveaux sont ordonnes par la distance qu'il faut parcourir pour les atteindre. On ne descend d'un cran que lorsque le précédent ne suffit pas."
      >
        ...
        <Callout tone="warning">
          Il existe un sixieme niveau : éditer le fichier. Il est legitime — la copie est
          ...
          la signalera, et il faudra la reporter a la main quand l’entrée amont evoluera.
      ...
      >
        <Callout tone="warning">
          La classe par défaut du composant est{' '}
          ...
          cascade. Entre deux règles de même specificite, c’est celle qui vient en dernier{' '}
...
// Marche toujours : en ligne, c’est la dernière écriture qui reste.
<ScrollProgress style={{ position: 'absolute' }} />

// Ce que la barre du haut de cette page reçoit reellement :
<ScrollProgress target={article} thickness={4} style={{ zIndex: 60 }} />`}
        />

        <p className="o-text-zinc-500 dark:o-text-zinc-400 o-max-w-prose">
          D’ou la place du niveau 3 dans l’échelle : marges, position, largeur,{' '}
      ...
      <Section
        ...
        lead="Un balisage convient rarement a deux maquettes. Le slot rend l'affichage remplacable sans toucher au calcul. La pastille en bas de la fenêtre en est un."
      >
        <CodeBlock
  ...
  {({ progress }) => (
    <span>lecture {Math.round(progress * 100)} %</span>
        ...
        <p className="o-text-zinc-500 dark:o-text-zinc-400 o-max-w-prose">
          Sans slot, la progression n’est jamais un état React : elle est écrite
          directement dans le <code className="o-font-mono o-text-sm">transform</code> de
          la barre, et le défilement ne provoque aucun rendu — c’est le cas de la barre du
      ...
      <Section
        ...
        lead="Sans elle, chaque besoin non prevu devient une propriété de plus. Au bout d'un an, le composant en a trente et la moitie ne sert qu'a un seul projet."
      >
...
/>`}
        ...
        <Callout>
          Le rappel est écrit en ligne, donc c’est une valeur neuve a chaque rendu du
          ...
          : la fonction est gardee dans une référence, et l’effet ne depend que de l’objet
          et de l’élément. Sans cela, chaque relevé poserait un intervalle de plus.
        </Callout>

        <p className="o-text-zinc-500 dark:o-text-zinc-400 o-max-w-prose">
          Le contexte porte aussi l’état du mouvement. L’echappatoire contourne l’API du
          composant, pas la préférence de l’utilisateur : sous mouvement réduit, le rappel
          ci-dessus ne pose aucun intervalle.
        </p>
      </Section>

      <Section
        title="Ce que la validation vérifie"
        lead="Trois règles sur cinq niveaux. Les deux autres se lisent, elles ne se mesurent pas — et un refus arbitraire sur du code correct est pire que pas de vérification : on apprend a contourner l'outil."
      >
        <div className="o-flex o-flex-col o-gap-3">
          <Niveau
            rang="1"
            titre="Les tokens déclarés et employes coincident"
          ...
          <Niveau
            rang="3"
            titre="Un composant qui rend un élément mentionne className"
          ...
          <Niveau
            rang="1"
            titre="Aucune couleur écrite en dur"
...
Registre invalide — 1 probleme(s) :
  ...
  · hooks/use-poster : le token --o-duration-slow est declare mais n'est
    employe nulle part.`}
        ...
        <p className="o-text-zinc-500 dark:o-text-zinc-400 o-max-w-prose">
          Cet exemple n’est pas invente : c’est ce que la règle a signale la première fois
          qu’elle a tourne, sur une entrée de ce registre. La déclaration etait restee
          après que la durée soit passee en propriété.
        </p>
      </Section>
    </div>
  )
