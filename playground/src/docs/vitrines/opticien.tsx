...
const VISAGES: readonly Morphologie[] = [
  {
    ...
    nom: 'Ovale',
    largeur: 137,
    trace: 'M100 20 C136 20 160 56 160 110 C160 176 130 244 100 244 C70 244 40 176 40 110 C40 56 64 20 100 20 Z',
    conseil: 'La forme qui accepte tout. On choisit alors par le gout, pas par la geometrie.',
  ...
  },
  {
    ...
    nom: 'Rond',
    largeur: 142,
    trace: 'M100 22 C142 22 160 60 160 120 C160 180 136 242 100 242 C64 242 40 180 40 120 C40 60 58 22 100 22 Z',
    conseil: 'On contrarie : des angles, une monture plus large que haute, un dessus marque.',
  ...
  },
  {
    ...
    nom: 'Carre',
    largeur: 145,
    trace: 'M44 34 C44 26 52 24 62 24 H138 C148 24 156 26 156 34 V152 C156 206 136 242 100 242 C64 242 44 206 44 152 Z',
  ...
  },
  {
    ...
    nom: 'Rectangle',
    largeur: 133,
    trace: 'M52 26 C52 22 60 20 70 20 H130 C140 20 148 22 148 26 V166 C148 218 130 250 100 250 C70 250 52 218 52 166 Z',
  ...
  },
  {
    ...
    nom: 'Coeur',
    largeur: 140,
    trace: 'M42 46 C42 30 60 22 100 22 C140 22 158 30 158 46 L150 140 C142 200 122 244 100 244 C78 244 58 200 50 140 Z',
    conseil: 'Le front porte, le menton est fin : une monture legere, plus lourde en bas qu en haut.',
  ...
  },
  {
    ...
    nom: 'Diamant',
    largeur: 136,
    trace: 'M100 20 C122 20 134 48 144 96 C154 148 128 246 100 246 C72 246 46 148 56 96 C66 48 78 20 100 20 Z',
    conseil: 'Les pommettes sont le point large : on elargit le front avec un dessus dessine.',
...
const VITRINE: readonly Monture[] = [
  { id: 'turbigo', nom: 'Turbigo', forme: 'rectangulaire', verre: 52, hauteur: 38, pont: 18, branche: 145, matiere: 'Acetate ecaille', prix: '185 EUR' },
  { id: 'volta', nom: 'Volta', forme: 'ronde', verre: 47, hauteur: 45, pont: 21, branche: 145, matiere: 'Metal dore mat', prix: '210 EUR' },
  { id: 'sedaine', nom: 'Sedaine', forme: 'papillon', verre: 54, hauteur: 42, pont: 17, branche: 140, matiere: 'Acetate miel', prix: '245 EUR' },
  { id: 'charonne', nom: 'Charonne', forme: 'carree', verre: 50, hauteur: 44, pont: 20, branche: 145, matiere: 'Acetate noir', prix: '165 EUR' },
  { id: 'laumiere', nom: 'Laumiere', forme: 'aviateur', verre: 56, hauteur: 46, pont: 16, branche: 145, matiere: 'Metal argent', prix: '230 EUR' },
  { id: 'buci', nom: 'Buci', forme: 'ovale', verre: 49, hauteur: 36, pont: 19, branche: 140, matiere: 'Titane fil', prix: '275 EUR' },
]

/** La monture de la carte de visite, et celle de l ouverture. */
const MARQUE: Monture = VITRINE[0] ?? { id: 'turbigo', nom: 'Turbigo', forme: 'rectangulaire', verre: 52, hauteur: 38, pont: 18, branche: 145, matiere: 'Acetate ecaille', prix: '185 EUR' }
...
function Reperes({ couleur, taille = 12 }: { readonly couleur: string; readonly taille?: number }): ReactElement {
  const coins = [
    ...
    <>
      {coins.map((coin) => (
        <span
          ...
          style={{ width: taille, height: taille, borderColor: couleur, borderStyle: 'solid', borderWidth: coin.bords }}
        />
      ))}
    </>
  )
 ...
 * d autre.
 */
function Cadran({ valeur, min, max }: { readonly valeur: number; readonly min: number; readonly max: number }): ReactElement {
  ...
  return (
    <svg viewBox="0 0 200 168" className="o-h-auto o-w-full" role="img" aria-label={`Cadran : ecart pupillaire de ${String(valeur)} millimetres`}>
      <path d={`M${x0.toFixed(1)} ${y0.toFixed(1)} A${String(rayon)} ${String(rayon)} 0 1 1 ${x1.toFixed(1)} ${y1.toFixed(1)}`} fill="none" stroke={accentDoux(700, 24)} strokeWidth="1" />
      {graduations.map((g) => (
        <g key={g.valeur}>
          <path d={`M${g.gx0.toFixed(1)} ${g.gy0.toFixed(1)} L${g.gx1.toFixed(1)} ${g.gy1.toFixed(1)}`} stroke={g.longue ? accentDoux(700, 52) : accentDoux(700, 26)} strokeWidth={g.longue ? 1.4 : 0.8} />
          {g.longue && (
            <text
      ...
      <path d={`M${String(centre)} ${String(centre)} L${ax.toFixed(1)} ${ay.toFixed(1)}`} stroke={accent(500)} strokeWidth="2.5" strokeLinecap="round" style={{ transition: 'none' }} />
      <circle cx={centre} cy={centre} r="4" fill={accent(500)} />
      <text x={centre} y={centre + 42} fontSize="26" textAnchor="middle" fill="currentColor" style={{ color: 'var(--o-theme-fg)', fontFamily: 'var(--o-font-mono)' }}>
        {valeur}
      </text>
      <text x={centre} y={centre + 56} fontSize="9" textAnchor="middle" fill="currentColor" style={{ color: 'var(--o-theme-muted)', fontFamily: 'var(--o-font-mono)', letterSpacing: '0.16em' }}>
        MM
      </text>
    </svg>
  )
...
const INDICES = [
  { indice: 1.5, nom: 'Organique 1,5', note: 'Le verre de base. Epais des que la correction monte.', prix: 'compris' },
  { indice: 1.6, nom: 'Aminci 1,6', note: 'Le bon compromis, de -2,00 a -4,00.', prix: '+ 60 EUR' },
  { indice: 1.67, nom: 'Aminci 1,67', note: 'Au-dela de -4,00, ou quand la monture est large.', prix: '+ 120 EUR' },
  { indice: 1.74, nom: 'Extra-aminci 1,74', note: 'Les fortes corrections, et les montures fines percees.', prix: '+ 210 EUR' },
] as const
...
const PICTOS: readonly Picto[] = [
  {
    titre: 'La mesure',
    legende: 'Ecart pupillaire, hauteur de montage, angle pantoscopique. Quinze minutes, sans rendez-vous.',
    trace: (
      <>
        <path d="M8 32h48" />
  ...
  {
    titre: 'Le montage',
    legende: 'Taille a la meuleuse dans l atelier du fond, sur le gabarit de votre monture.',
    trace: (
      <>
  ...
  {
    titre: 'Le reglage',
    legende: 'A vie, et gratuit, meme si la monture vient d ailleurs. Une branche tordue se redresse.',
    trace: (
      <>
  ...
  {
    titre: 'La garantie',
    legende: 'Deux ans sur la monture, un an sur l adaptation : si vous ne vous y faites pas, on refait.',
    trace: (
      <>
...
function Essai({ ecart }: { readonly ecart: number }): ReactElement {
  ...
  const couleurTrait =
    releve.verdict === 'deconseillee' ? 'light-dark(var(--o-palette-rose-600), var(--o-palette-rose-400))' : releve.verdict === 'conseillee' ? accent(500) : accentDoux(900, 55)
    ...
    <div className="o-grid o-gap-10 lg:o-grid-cols-12">
      ...
      <div className="lg:o-col-span-5">
        <div className="o-relative o-p-6" style={{ backgroundColor: accentDoux(300, 8) }}>
          ...
          <svg viewBox="0 0 200 264" className="o-mx-auto o-h-auto o-w-full" style={{ maxWidth: 320 }} role="img" aria-label={`Visage ${forme.nom.toLowerCase()} portant la monture ${piece.nom}`}>
            {/* La tete, au trait. */}
            <path d={forme.trace} fill="var(--o-theme-bg)" stroke={accentDoux(900, 46)} strokeWidth="1.4" />
            ...
            <text x="100" y="150" fontSize="7" textAnchor="middle" fill="currentColor" style={{ color: 'var(--o-theme-muted)', fontFamily: 'var(--o-font-mono)', letterSpacing: '0.1em' }}>
            ...
            <g transform={`translate(100 112) scale(${echelle.toFixed(4)}) translate(-100 -112)`}>
              <Face monture={piece} cx={100} cy={112} trait={2.2 / echelle} couleur={couleurTrait} />
            </g>
          </svg>
        </div>
      </div>
      ...
      <div className="o-min-w-0 lg:o-col-span-7">
        <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400">1 — La forme du visage</p>
        {/*
        ...
        */}
        <ul className="o-m-0 o-mt-4 o-grid o-list-none o-grid-cols-3 o-gap-px o-p-0 sm:o-grid-cols-6" style={{ backgroundColor: accentDoux(700, 20) }}>
          {VISAGES.map((forme2) => {
              ...
              <li key={forme2.id}>
                ...
                >
                  <svg viewBox="20 10 160 250" className="o-w-10" aria-hidden="true">
                    <path d={forme2.trace} fill="none" stroke="currentColor" strokeWidth="6" />
                  </svg>
                  <span className="o-font-mono o-text-xs o-uppercase o-tracking-widest">{forme2.nom}</span>
                </button>
              </li>
            )
          })}
        </ul>
        <p className="o-m-0 o-mt-4 o-max-w-md o-text-sm o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">{forme.conseil}</p>

        <p className="o-m-0 o-mt-10 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400">2 — La monture</p>
        <ul className="o-m-0 o-mt-4 o-grid o-list-none o-gap-px o-p-0 sm:o-grid-cols-3" style={{ backgroundColor: accentDoux(700, 20) }}>
          {VITRINE.map((piece2) => {
                ...
                <GlareHover duration={620} width={18} className="o-h-full">
                  ...
                  >
                    <svg viewBox="0 0 140 60" className="o-w-full" aria-hidden="true">
                      <Face monture={piece2} cx={70} cy={30} trait={1.6} couleur="currentColor" />
                    </svg>
                    <span className="o-font-mono o-text-xs o-uppercase o-tracking-widest">{piece2.nom}</span>
        ...
        <div className="o-relative o-mt-10 o-p-6" style={{ boxShadow: `inset 0 0 0 1px ${accentDoux(700, 30)}` }}>
          <Reperes couleur={accent(500)} taille={9} />
          <p className="o-m-0 o-flex o-flex-wrap o-items-baseline o-gap-3">
            <span className="o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400">Verdict</span>
            <span
          ...
          <dl className="o-m-0 o-mt-6 o-grid o-gap-6 sm:o-grid-cols-3">
            {([
              ['Largeur frontale', `${String(releve.largeur)} mm`, releve.debord > 2 ? `deborde de ${String(releve.debord)} mm` : releve.debord < -6 ? `etroite de ${String(-releve.debord)} mm` : 'bien posee'],
              ['Centres optiques', `${String(releve.optique)} mm`, `votre ecart : ${String(ecart)} mm`],
              ['Decentrement', `${releve.decentrement.toFixed(1).replace('.', ',')} mm`, releve.decentrement > 2 ? 'verres plus epais au bord' : 'sans consequence'],
              ...
              <div key={quoi}>
                ...
                <dd className="o-m-0 o-mt-2 o-font-mono o-text-xl o-tabular-nums o-text-zinc-950 dark:o-text-zinc-50">{valeur}</dd>
          ...
          <p className="o-m-0 o-mt-6 o-border-t o-border-black-10 dark:o-border-zinc-800 o-pt-4 o-text-xs o-leading-relaxed o-text-zinc-500 dark:o-text-zinc-400">
            ...
            metier : on contrarie la forme du visage, on ne la repete pas. Il ne
            remplace pas l essai, il vous evite d essayer douze montures.
          </p>
        </div>
      </div>
    </div>
      ...
      >
        ...
        >
          <div className="o-flex o-items-start o-justify-between o-gap-6">
            <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400">Opticien — Paris 3</p>
          ...
          <p
            ...
            style={{ ...affiche('m', 300), fontSize: 'clamp(1.5rem, 3vw, 2.25rem)', lineHeight: 1 }}
          >
            Verre &amp; Monture
          </p>
        </div>
        ...
        >
          ...
          <div className="o-flex o-flex-wrap o-items-center o-gap-4">
            ...
            <span className="o-font-mono o-text-xs o-uppercase o-tracking-widest" style={{ color: encreSurSombre() }}>
              Sans rendez-vous
            </span>
          </div>
        </div>
...
export default function Page(): ReactElement {
  ...
  const verres = useMemo(() => INDICES.map((ligne) => ({ ...ligne, bord: bordDuVerre(ligne.indice) })), [])
    ...
    <Porte forme="iris" marque="Verre &amp; Monture" sombre={false}>
      <div className="o-bg-zinc-50 dark:o-bg-zinc-950 o-text-zinc-900 dark:o-text-zinc-50" style={polices}>
        ...
        <header className="o-relative o-isolate o-flex o-flex-col" style={{ minHeight: ECRAN }}>
          ...
          <BarreCoins marque="Verre &amp; Monture" liens={NAVIGATION} droite="Paris 3 — sans rendez-vous" sombre={false} />
          ...
          <div className="o-relative o-flex o-grow o-flex-col o-justify-center o-gap-12 o-px-6 o-py-12 md:o-px-12">
            <div className="o-grid o-items-center o-gap-12 lg:o-grid-cols-12">
              <div className="lg:o-col-span-7">
                <Surgit>
                  <Etiquette sombre={false}>Opticien lunetier — atelier au fond du magasin</Etiquette>
                </Surgit>
                <TitreVague
                  delai={140}
                  className="o-m-0 o-mt-7 o-text-zinc-950 dark:o-text-zinc-50"
                  style={{ ...affiche('l', 300), fontSize: 'clamp(2.5rem, 8vw, 7.5rem)', lineHeight: 0.9, letterSpacing: '-0.04em' }}
                >
                  Verre &amp; Monture
                </TitreVague>
                <Surgit delai={520} as="p" className="o-m-0 o-mt-8 o-max-w-lg o-text-base o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">
                  Une monture se choisit en millimetres avant de se choisir en
                  gout. Posez la votre sur votre visage, ici, avant de pousser la
                  porte.
                </Surgit>
                <Surgit delai={640} className="o-mt-10 o-flex o-flex-wrap o-items-center o-gap-4">
                  <Aimant force={0.4}>
                    <a
                      href="#essai"
                      ...
                      style={aplat()}
                    >
                      Essayer une monture <Icon icon={ArrowRight} size={14} aria-hidden="true" />
                    </a>
                  </Aimant>
                  <a
                    href="#verres"
                    ...
                    style={{ boxShadow: `inset 0 0 0 1px ${accentDoux(700, 40)}`, color: 'inherit' }}
                  >
              ...
              <Surgit delai={340} className="lg:o-col-span-5">
                <div className="o-relative o-p-6" style={{ boxShadow: `inset 0 0 0 1px ${accentDoux(700, 26)}` }}>
                  <Reperes couleur={accent(500)} taille={10} />
                  <svg viewBox="0 0 150 86" className="o-h-auto o-w-full" role="img" aria-label="La monture Sedaine, cotee : verre 54, pont 17, branche 140">
                    <g stroke={accentDoux(700, 30)} strokeWidth="0.4" strokeDasharray="2 2">
                      <path d="M21 22v-10M75 22v-10M129 22v-10" />
                      <path d="M21 14h54M75 14h54" />
                    </g>
                    <text x="48" y="11" fontSize="5" textAnchor="middle" fill="currentColor" style={{ color: 'var(--o-theme-muted)', fontFamily: 'var(--o-font-mono)', letterSpacing: '0.1em' }}>
                      54 MM
                    </text>
                    <text x="102" y="11" fontSize="5" textAnchor="middle" fill="currentColor" style={{ color: 'var(--o-theme-muted)', fontFamily: 'var(--o-font-mono)', letterSpacing: '0.1em' }}>
                      17 MM
                    </text>
                    <g transform="translate(75 48) scale(0.86) translate(-75 -48)">
                      <Face monture={VEDETTE} cx={75} cy={48} trait={1.8} couleur={accentDoux(900, 60)} />
                    </g>
                  </svg>
        ...
        <section aria-labelledby="manifeste-titre" className="o-border-t o-border-black-10 dark:o-border-zinc-800 o-px-6 o-py-24 md:o-px-12 md:o-py-32">
          <div className="o-mx-auto o-max-w-5xl">
            ...
            <Reveal>
              <TrueFocus
                ...
                style={{ ...affiche('m', 300), fontSize: 'clamp(1.6rem, 4vw, 3.5rem)', lineHeight: 1.08 }}
              >
            ...
            <Reveal delay={120}>
              <p className="o-m-0 o-mt-10 o-max-w-xl o-text-base o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">
                Quatre gestes, et trois d entre eux se font apres la vente. C est
                la difference entre un opticien et un rayon de lunettes : ce qui
                vous est vendu n est pas la monture, c est l ajustement.
              </p>
            </Reveal>
          </div>
        </section>
        ...
        {/* ================= Le mecanisme : l essai ======================= */}
        <section id="essai" className="o-scroll-mt-24 o-border-t o-border-black-10 dark:o-border-zinc-800 o-px-6 o-py-24 md:o-px-12 md:o-py-32">
        ...
        <section id="ecart" className="o-scroll-mt-24 o-border-t o-border-black-10 dark:o-border-zinc-800 o-px-6 o-py-24 md:o-px-12 md:o-py-32">
          <div className="o-mx-auto o-grid o-max-w-6xl o-items-center o-gap-12 lg:o-grid-cols-12">
            <div className="lg:o-col-span-7">
              ...
              <p className="o-m-0 o-mt-6 o-max-w-xl o-text-base o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">
                Le centre optique du verre doit tomber devant la pupille. S il
                tombe a cote, le verre devient un prisme : la tete tire, et on
                accuse la correction. Reglez votre ecart ci-dessous —
                l essai plus haut en tient compte aussitot.
              </p>
              ...
              <p className="o-m-0 o-mt-6 o-max-w-md o-text-xs o-leading-relaxed o-text-zinc-500 dark:o-text-zinc-400">
                Un adulte se tient entre cinquante-huit et soixante-huit
                millimetres. Nous le mesurons au pupillometre, en quinze
                secondes, meme si vous n achetez rien.
              </p>
            </div>
        ...
        {/* ================= Les verres, en coupe ========================= */}
        <section id="verres" className="o-scroll-mt-24 o-px-6 o-py-24 md:o-px-12 md:o-py-32" style={nuit('slate')}>
          <div className="o-mx-auto o-max-w-6xl">
            <Reveal>
            ...
            <p className="o-m-0 o-mt-6 o-max-w-xl o-text-base o-leading-relaxed o-text-slate-300">
              Quatre verres pour la meme correction de -4,00 dioptries, dessines
              a la meme echelle. Ce qui change n est pas le centre, c est le
              bord : c est lui qu on voit dans la monture.
            </p>

            <ul className="o-m-0 o-mt-14 o-grid o-list-none o-gap-px o-p-0 sm:o-grid-cols-2 lg:o-grid-cols-4" style={{ backgroundColor: 'var(--o-palette-slate-800)' }}>
              {verres.map((verre) => {
                ...
                return (
                  <li key={verre.nom} className="o-bg-slate-950 o-px-5 o-py-6">
                    ...
                    */}
                    <svg viewBox="0 0 30 8" className="o-mt-5 o-h-auto o-w-full" role="img" aria-label={`Demi-coupe du verre d indice ${String(verre.indice)} : ${bord.toFixed(1)} millimetres au bord, un millimetre au centre`}>
                      <path
                      ...
                      <path d="M2 0.6V7.4" stroke={accentDoux(400, 40)} strokeWidth="0.16" strokeDasharray="0.6 0.6" />
                      <path d={`M28.4 ${(4 - demi).toFixed(2)}v${bord.toFixed(2)}`} stroke={accent(400)} strokeWidth="0.24" />
                      <path d={`M27 ${(4 - demi).toFixed(2)}h1.8M27 ${(4 + demi).toFixed(2)}h1.8`} stroke={accent(400)} strokeWidth="0.14" />
                    </svg>
                    ...
                    <p className="o-m-0 o-mt-4 o-border-t o-border-white-10 o-pt-3 o-text-xs o-leading-relaxed o-text-slate-300">{verre.note}</p>
                    <p className="o-m-0 o-mt-3 o-font-mono o-text-xs o-uppercase o-tracking-widest" style={{ color: encreSurSombre() }}>
                      {verre.prix}
                    </p>
                  </li>
                )
            ...
            <p className="o-m-0 o-mt-8 o-max-w-2xl o-text-xs o-leading-relaxed o-text-slate-400">
              Calcul simplifie, et dit comme tel : une seule face courbee, un
              verre de cinquante millimetres, un centre d un millimetre. Les
              valeurs absolues varient avec la monture ; l ecart entre les
              indices, lui, est celui-la.
            </p>
          </div>
        </section>

        {/* ================= L appel : la carte de visite (A24) =========== */}
        <section id="carte" className="o-scroll-mt-24 o-px-6 o-py-24 md:o-px-12 md:o-py-32">
        ...
        <footer className="o-border-t o-border-black-10 dark:o-border-zinc-800 o-px-6 o-py-16 md:o-px-12">
          <div className="o-mx-auto o-max-w-6xl">
            <ul className="o-m-0 o-grid o-list-none o-gap-px o-p-0 sm:o-grid-cols-2" style={{ backgroundColor: accentDoux(700, 18) }}>
              {PICTOS.map((picto) => (
                <li key={picto.titre} className="o-flex o-items-start o-gap-6 o-bg-zinc-50 dark:o-bg-zinc-950 o-px-5 o-py-6">
                  {/* Le pictogramme dans son cadre de reperage : c est le meme
                      vocabulaire que le reste de la page, pas une icone posee. */}
                  <span className="o-relative o-block o-shrink-0" style={{ width: 76, height: 76, boxShadow: `inset 0 0 0 1px ${accentDoux(700, 26)}` }}>
                    <Reperes couleur={accent(500)} taille={7} />
                    <svg viewBox="0 0 64 64" className="o-size-full" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" style={{ color: encre() }}>
                      {picto.trace}
                    </svg>
                  </span>
                  <span>
                    <h2 className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-950 dark:o-text-zinc-50">{picto.titre}</h2>
                    <p className="o-m-0 o-mt-3 o-text-sm o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">{picto.legende}</p>
                  </span>
                </li>
              ))}
            </ul>
            ...
            <div className="o-mt-12 o-flex o-flex-wrap o-items-center o-justify-between o-gap-6 o-border-t o-border-black-10 dark:o-border-zinc-800 o-pt-6">
              ...
              <a
                ...
                style={{ color: encre() }}
              >
                atelier@verre-et-monture.fr <Icon icon={ArrowUpRight} size={14} aria-hidden="true" />
              </a>
            </div>
          </div>
        </footer>
