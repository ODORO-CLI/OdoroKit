...
import { Actions, affiche, BarreFilet, CHROME, Coin, Etiquette, Grain, Horloge, Indice, Porte, Surgit, TitreVague, usePolices } from './marche.jsx'
...
const MODELES: readonly Modele[] = [
  { nom: 'halo-clair', famille: 'Texte', resume: 'Le modele de tous les jours : classement, reecriture, extraction. Repond en flux des le premier jeton.', entree: 0.25, sortie: 1.1, fenetre: '128 k jetons', marque: null },
  { nom: 'halo-ample', famille: 'Texte', resume: 'Le raisonnement long, les documents entiers, les chaines d outils. C est celui que la plupart des equipes finissent par choisir.', entree: 2.4, sortie: 9.6, fenetre: '400 k jetons', marque: 'Le plus demande' },
  { nom: 'halo-vue', famille: 'Vision', resume: 'Lit des captures, des tableaux photographies et des plans. Une image vaut 850 jetons en entree.', entree: 1.8, sortie: 7.2, fenetre: '200 k jetons', marque: null },
  { nom: 'halo-voix', famille: 'Audio', resume: 'Transcription et synthese, facturees a la minute plutot qu au jeton. Trente-huit langues reconnues.', entree: 0.9, sortie: 3.4, fenetre: '60 min par requete', marque: null },
]

/** Les quatre phases de la scene epinglee : ce que la meme cle ouvre. */
const PHASES: readonly { readonly titre: string; readonly modeles: string; readonly corps: string; readonly chiffre: string }[] = [
  { titre: 'Texte', modeles: 'halo-clair, halo-ample', corps: 'Reponse en flux des le premier jeton, sortie contrainte a un schema JSON, et un mode « raisonnement long » qui expose son brouillon quand vous le demandez.', chiffre: 'Une equipe de support classe 40 000 messages par jour a 0,004 EUR l unite.' },
  { titre: 'Vision', modeles: 'halo-vue', corps: 'Captures d ecran, tableaux photographies, plans scannes : le modele rend la structure, pas seulement le texte.', chiffre: 'Une image de 1024 par 1024 compte 850 jetons en entree.' },
  { titre: 'Voix', modeles: 'halo-voix', corps: 'Transcription horodatee et synthese vocale, trente-huit langues, facturees a la minute d audio.', chiffre: 'Une heure d entretien transcrite coute 0,54 EUR.' },
  { titre: 'Outils', modeles: 'Toutes familles', corps: 'Vous declarez vos fonctions, le modele choisit laquelle appeler et avec quels arguments. Les appels paralleles sont pris en charge.', chiffre: 'Jusqu a 128 outils declares dans une meme conversation.' },
]
...
const ETAPES: readonly { readonly x: number; readonly nom: string; readonly sous: string; readonly duree: string; readonly delai: string }[] = [
        ...
        >
          ...
          {ETAPES.map((etape) => (
            <g key={etape.nom}>
              ...
              />
              <rect x={etape.x} y={96} width={140} height={66} rx={10} fill="none" stroke={filet} strokeWidth="1.2" />
              <text x={etape.x + 14} y={124} fontSize="15" fontWeight="600" fill="currentColor">{etape.nom}</text>
              <text x={etape.x + 14} y={144} fontSize="9.5" fontFamily="ui-monospace, monospace" fill={pale}>{etape.sous}</text>
              <text x={etape.x + 14} y={188} fontSize="12" fontFamily="ui-monospace, monospace" fill={ENCRE}>{etape.duree}</text>
            </g>
          ))}

          <text x={726} y={256} fontSize="12" fontWeight="600" fill="currentColor">Vos outils</text>
          <text x={726} y={272} fontSize="9.5" fontFamily="ui-monospace, monospace" fill={pale}>appeles par le modele, puis rendus</text>
          <text x={706} y={306} fontSize="10" fontFamily="ui-monospace, monospace" fill={pale}>Un appel d outil ajoute un aller-retour complet.</text>
          <text x={6} y={62} fontSize="11" fontFamily="ui-monospace, monospace" fill={pale}>Le premier jeton arrive en 323 ms — mediane, region eu-par</text>
  ...
  return (
    <figure className="o-m-0 o-grid o-gap-8 lg:o-grid-cols-12 lg:o-gap-12">
      <figcaption className="lg:o-col-span-3">
        <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest" style={{ color: ENCRE }}>Figure {rang}</p>
        <p className="o-m-0 o-mt-4 o-text-lg o-font-semibold o-leading-snug o-text-zinc-950 dark:o-text-zinc-50">{titre}</p>
        <p className="o-m-0 o-mt-4 o-text-sm o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">{legende}</p>
      </figcaption>
  ...
  return (
    <ul ref={poser} className="o-m-0 o-list-none o-p-0">
      {MODELES.map((modele, rang) => (
        <li key={modele.nom} className="o-grid o-gap-x-8 o-gap-y-3 o-border-b o-border-black-10 dark:o-border-zinc-800 o-py-8 md:o-grid-cols-12 md:o-items-baseline">
          ...
          <div className="md:o-col-span-4">
            <h3 className="o-m-0 o-font-mono o-text-xl o-font-semibold o-tracking-tight">{modele.nom}</h3>
            <p className="o-m-0 o-mt-1 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400">
              ...
              {modele.marque !== null && <span className="o-ml-2" style={{ color: ENCRE }}>· {modele.marque}</span>}
            </p>
          </div>
          <p className="o-m-0 o-text-sm o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400 md:o-col-span-4">{modele.resume}</p>
          <div className="md:o-col-span-3">
            ...
            <div aria-hidden="true" className="o-mt-3 o-h-1.5 o-w-full o-overflow-hidden o-rounded-full" style={{ backgroundColor: FILET }}>
              <div
...
const EXEMPLES: readonly Exemple[] = [
  ...
  },
  {
    ...
    invite: 'Extrais le montant, la date et le fournisseur de cette facture, au format JSON strict.',
    modele: 'halo-clair',
    reponse: [
  ...
  },
  {
    ...
    invite: 'Quelle est la disponibilite de la reference AX-4120 dans l entrepot de Nantes ?',
    modele: 'halo-ample',
    reponse: [
...
function BacASable(): ReactElement {
  ...
  const cout = choisi !== undefined && jetons !== null ? (jetons.entree * choisi.entree + jetons.sortie * choisi.sortie) / 1_000_000 : 0

  return (
    <>
      <div className="o-flex o-flex-wrap o-items-center o-gap-x-6 o-gap-y-3">
        <div role="group" aria-label="Exemples predefinis" className="o-flex o-flex-wrap o-gap-2">
      ...
      <div className="o-mt-6">
        <PromptInput
          ...
          controls={<span className="o-font-mono o-text-xs o-text-zinc-600 dark:o-text-zinc-400">{modele} · temperature 0,2</span>}
        />
      </div>

      <div aria-live="polite" className="o-mt-6 o-text-sm o-leading-relaxed o-text-zinc-900 dark:o-text-zinc-50">
        {invite === null ? (
          <p className="o-m-0 o-max-w-3xl o-pl-6 o-text-zinc-600 dark:o-text-zinc-400" style={{ borderLeft: `2px solid ${FILET}` }}>
            Choisissez un exemple ci-dessus, ou posez votre question. Aucun appel n est emis depuis cette page : les reponses sont fixes, et servent a montrer la forme d une reponse et le decompte qui l accompagne.
          </p>
        ) : (
          <div className="o-max-w-3xl o-pl-6" style={{ borderLeft: `2px solid ${accent(500)}` }}>
            <p className="o-m-0 o-font-mono o-text-xs o-text-zinc-600 dark:o-text-zinc-400">Envoye a {modele} — « {invite} »</p>
            ...
            {jetons !== null && (
              <p className="o-m-0 o-mt-4 o-font-mono o-text-xs o-text-zinc-600 dark:o-text-zinc-400">
                {nombre(jetons.entree)} jetons en entree, {nombre(jetons.sortie)} en sortie — <span style={{ color: ENCRE }}>{nombre(cout, 4)} EUR</span> pour cet appel sur {modele}.
              </p>
            )}
          </div>
        )}
...
function Calculateur(): ReactElement {
  ...
  const moinsCher = lignes.reduce((bas, ligne) => (ligne.total < bas.total ? ligne : bas), lignes[0] as (typeof lignes)[number])
  const economie = cache ? lignes.reduce((somme, ligne) => somme + entreeM * (1 - PART_CACHE) * ligne.modele.entree, 0) / lignes.length : 0

  return (
    <div>
      <div className="o-grid o-gap-8 md:o-grid-cols-2">
        <div>
          <label htmlFor="calc-jetons" className="o-block o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-600 dark:o-text-zinc-400">
            Jetons d entree par mois
          </label>
          <p className="o-m-0 o-mt-1 o-font-mono o-text-3xl o-font-bold o-tabular-nums o-tracking-tight">{nombre(entreeM)} M</p>
          <input
            id="calc-jetons"
            type="range"
            min={0}
          ...
          />
          <p aria-hidden="true" className="o-m-0 o-mt-2 o-flex o-justify-between o-font-mono o-text-xs o-tabular-nums o-text-zinc-600 dark:o-text-zinc-400">
            <span>1 M</span>
            <span>200 M</span>
            <span>20 000 M</span>
          </p>
        ...
        <div>
          <p id="forme-titre" className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-600 dark:o-text-zinc-400">Longueur des reponses</p>
          ...
          <p className="o-m-0 o-mt-3 o-font-mono o-text-xs o-text-zinc-600 dark:o-text-zinc-400">
            {FORMES.find((f) => f.id === forme)?.exemple ?? ''} — soit {nombre(sortieM)} M jetons de sortie par mois.
          </p>
          ...
          <label className="o-mt-6 o-flex o-cursor-pointer o-items-start o-gap-3 o-text-sm">
            ...
            />
            <span>
              <span className="o-block o-font-medium">Instructions systeme mises en cache</span>
              <span className="o-block o-text-xs o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">Soixante pour cent d une entree reelle se repete d un appel a l autre. Le cache la facture a un dixieme du prix.</span>
            </span>
          </label>
        </div>
      </div>
      ...
      <div className="o-mt-10 o-overflow-x-auto">
        <table className="o-w-full o-text-sm">
          <caption className="o-sr-only">Cout mensuel des quatre modeles pour {nombre(entreeM)} millions de jetons d entree</caption>
          <thead>
            <tr>
              {['Modele', 'Entree', 'Sortie', 'Par mois'].map((entete, rangEntete) => (
                <th key={entete} scope="col" className={`o-py-3 o-pr-4 o-font-mono o-text-xs o-font-normal o-uppercase o-tracking-widest o-text-zinc-600 dark:o-text-zinc-400 ${rangEntete === 0 ? 'o-text-left' : 'o-text-right'}`} style={{ borderBottom: `1px solid ${FILET}` }}>
                  {entete}
                </th>
              ))}
            </tr>
            ...
            {lignes.map((ligne) => {
              ...
              return (
                <tr key={ligne.modele.nom}>
                  <th scope="row" className="o-py-4 o-pr-4 o-text-left o-font-mono o-text-sm o-font-semibold o-whitespace-nowrap" style={{ borderBottom: `1px solid ${FILET}` }}>
                    {ligne.modele.nom}
                    {bas && <span className="o-ml-2 o-rounded-sm o-px-1.5 o-py-0.5 o-text-xs o-font-normal" style={{ backgroundColor: VOILE, color: ENCRE }}>le moins cher</span>}
                  </th>
                  <td className="o-py-4 o-pr-4 o-text-right o-tabular-nums o-text-zinc-600 dark:o-text-zinc-400" style={{ borderBottom: `1px solid ${FILET}` }}>{nombre(ligne.coutEntree, 2)} EUR</td>
                  <td className="o-py-4 o-pr-4 o-text-right o-tabular-nums o-text-zinc-600 dark:o-text-zinc-400" style={{ borderBottom: `1px solid ${FILET}` }}>{nombre(ligne.coutSortie, 2)} EUR</td>
                  <td className="o-py-4 o-pr-4 o-text-right o-font-mono o-font-semibold o-tabular-nums" style={{ borderBottom: `1px solid ${FILET}`, color: bas ? ENCRE : undefined }}>{nombre(ligne.total, 2)} EUR</td>
                </tr>
              )
            })}
          </tbody>
      ...
      <p aria-live="polite" className="o-m-0 o-mt-6 o-max-w-2xl o-text-sm o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">
        {cache
...
function Phase({ rang }: { readonly rang: number }): ReactElement {
  ...
  return (
    <div className="o-max-w-xl">
      ...
      <p className="o-m-0 o-mt-6 o-max-w-md o-text-base o-leading-relaxed" style={{ color: accent(100) }}>
        {phase.corps}
      </p>
      <p className="o-m-0 o-mt-5 o-max-w-md o-border-l o-pl-4 o-font-mono o-text-xs o-leading-relaxed" style={{ color: accent(200), borderColor: accent(400) }}>
        {phase.chiffre}
      </p>
    </div>
  )
...
function Accroche({ visible }: { readonly visible: boolean }): ReactElement {
  return (
    <div className={visible ? 'o-max-w-5xl' : 'o-sr-only'}>
      ...
      <TitreVague delai={120} className="o-m-0 o-mt-8" style={{ ...affiche('l', 800), color: accent(50) }}>
        Essayez le modele avant de lire son prix.
      </TitreVague>
      <Surgit delai={560} as="p" className="o-m-0 o-mt-8 o-max-w-lg o-text-base o-leading-relaxed md:o-text-lg" style={{ color: accent(100) }}>
        Une interface, une facture au jeton lisible a la ligne, et un bac a sable avant la moindre promesse.
      </Surgit>
      <Surgit delai={700} className="o-mt-10">
        <Actions pleine={['#essai', <>Essayer maintenant <Icon icon={ArrowRight} size={16} aria-hidden="true" /></>]} fantome={['#modeles', 'Voir les prix']} />
      </Surgit>
    </div>
  )
}
...
export default function Page(): ReactElement {
  ...
  const scene = (acte: number): ReactNode => (
    <div className="o-relative o-flex o-h-full o-flex-col" style={{ backgroundColor: accent(950), color: accent(50) }}>
      ...
      <div
        ...
        style={{
          transform: acte === 0 ? 'translate3d(0, 0, 0) scale(1)' : `translate3d(22%, 0, 0) scale(${(1.12 + acte * 0.1).toFixed(2)})`,
          transition: reduced ? undefined : 'transform 1100ms cubic-bezier(0.16, 1, 0.3, 1)',
        }}
      >
        {sphere}
      </div>
      ...
      <BarreFilet marque={<span className="o-inline-flex o-items-center o-gap-2"><Icon icon={Sparkles} size={14} style={{ color: accent(300) }} aria-hidden="true" />Halo</span>} liens={LIENS} action={['#cle', 'Console']} />
      ...
      <p aria-hidden="true" className="o-pointer-events-none o-absolute o-bottom-6 o-left-1/2 o-m-0 o-flex o-items-center o-gap-3 o-font-mono o-text-xs o-tabular-nums o-uppercase o-tracking-widest" style={{ transform: 'translateX(-50%)', color: accent(300) }}>
        {[0, 1, 2, 3, 4].map((rang) => (
          <span key={rang} className="o-block o-h-1 o-rounded-full o-transition-all" style={{ width: rang === acte ? 28 : 8, backgroundColor: rang === acte ? accent(300) : `color-mix(in oklab, ${accent(300)} 35%, transparent)` }} />
        ))}
      </p>
      <Coin position="bd">Paris et Francfort<br />Aucun jeton ne quitte la region</Coin>
    </div>
  )
  ...
  return (
    <Porte forme="compteur" marque="Halo">
      <div className="o-bg-white dark:o-bg-zinc-950 o-text-zinc-900 dark:o-text-zinc-50" style={polices}>
        ...
        {reduced ? (
          <section aria-label="Ouverture" className="o-relative o-isolate o-overflow-hidden" style={{ backgroundColor: accent(950), color: accent(50) }}>
            <div aria-hidden="true" className="o-absolute o-inset-0">{sphere}</div>
            <div aria-hidden="true" className="o-absolute o-inset-0" style={{ background: `linear-gradient(to top, ${accent(950)} 10%, transparent 60%)` }} />
        ...
        <main>
          {/* =================================================================
              ...
              ================================================================= */}
          <section id="trajet" aria-labelledby="trajet-titre" className="o-scroll-mt-24 o-px-6 o-py-20 md:o-py-28" style={{ backgroundColor: VOILE }}>
            <div className="o-mx-auto o-max-w-7xl">
              <h2 id="trajet-titre" className="o-sr-only">Le trajet d une requete</h2>
              <Figure
                rang="1"
          ...
          <section id="essai" className="o-scroll-mt-24 o-px-6 o-py-24 md:o-py-32">
            <div className="o-mx-auto o-max-w-5xl">
              <Indice rang="01" sombre={false}>Le bac a sable</Indice>
              <h2 className="o-m-0 o-mt-5 o-flex o-items-center o-gap-3 o-text-zinc-950 dark:o-text-zinc-50" style={{ ...affiche('m', 800), fontSize: 'clamp(2rem, 4.5vw, 4.25rem)' }}>
                <Icon icon={Terminal} size={28} style={{ color: ENCRE }} aria-hidden="true" />
              ...
              <p className="o-mt-5 o-max-w-xl o-text-base o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">
                Un raisonnement, une extraction contrainte, un appel d outil, une transcription : les quatre formes de reponse que l interface sait rendre.
              </p>
          ...
          {/* =================================================================
              ...
              ================================================================= */}
          <section aria-labelledby="facture-titre" className="o-flex o-items-center o-border-t o-border-black-10 dark:o-border-zinc-800 o-px-6 o-py-28 md:o-py-40" style={{ minHeight: '74vh' }}>
            <div className="o-mx-auto o-w-full o-max-w-5xl">
              <h2 id="facture-titre" className="o-sr-only">Ce que coute reellement un modele</h2>
              <ScrollReveal
                as="p"
                course={0.85}
                dim={0.14}
                className="o-m-0 o-text-zinc-950 dark:o-text-zinc-50"
                style={{ ...affiche('m', 800), fontSize: 'clamp(1.75rem, 4.4vw, 4rem)', lineHeight: 1.08 }}
              >
                Le prix d un modele ne se lit pas au million de jetons. Il se lit a la fin du mois, quand le routage, le cache et la longueur de vos reponses ont fait leur part.
              </ScrollReveal>
            </div>
          </section>
          ...
          {/* =================================================================
              ...
              les deux themes. Une facture ne se lit pas sur du papier blanc.
              ================================================================= */}
          <section aria-labelledby="prix-titre" className="o-relative o-isolate o-overflow-hidden o-px-6 o-py-24 md:o-py-36" style={nuit('zinc')}>
            <Grain opacite={0.05} />
            <div className="o-relative o-mx-auto o-max-w-7xl o-text-zinc-50">
              <h2 id="prix-titre" className="o-sr-only">Le prix du million de jetons</h2>
              <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-400">Le million de jetons d entree, sur halo-clair</p>
              <p className="o-m-0 o-mt-6 o-font-mono o-tabular-nums o-tracking-tighter" style={{ fontFamily: 'var(--o-vitrine-affichage)', fontSize: 'clamp(5rem, 17vw, 15rem)', fontWeight: 800, lineHeight: 0.85, letterSpacing: '-0.05em' }}>
              ...
              <div className="o-mt-10 o-grid o-gap-6 md:o-grid-cols-12">
                <p className="o-m-0 o-max-w-xl o-text-xl o-leading-snug md:o-col-span-7 md:o-text-2xl">
                  Pas de minimum mensuel, pas de palier cache : l entree et la sortie, comptees a la fin de chaque appel.
                </p>
                <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-leading-relaxed o-tracking-widest o-text-zinc-400 md:o-col-span-5 md:o-text-right">
                  Hors taxes, facture en fin de mois<br />Grille publiee ci-dessous, au centime
                </p>
              </div>
            </div>
          </section>
          ...
          <section id="modeles" className="o-scroll-mt-24 o-border-t o-border-black-10 dark:o-border-zinc-800 o-px-6 o-py-24 md:o-py-32">
            <div className="o-mx-auto o-max-w-7xl">
              <Indice rang="02" sombre={false}>Les quatre modeles</Indice>
              <ParticleText as="h2" step={4} duration={1600} className="o-m-0 o-mt-5 o-text-zinc-950 dark:o-text-zinc-50" style={affiche('xl', 800)}>
                Ensemble.
              </ParticleText>
              <p className="o-mt-6 o-max-w-xl o-text-base o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">
                Une seule cle ouvre les quatre. La facture detaille chaque modele separement, et l ecart entre eux decide d une architecture de routage.
              </p>
              ...
              <p className="o-mt-3 o-max-w-2xl o-font-mono o-text-xs o-uppercase o-leading-relaxed o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400">
                La barre donne le prix de sortie, a l echelle du plus cher des quatre — les jetons d entree mis en cache sont factures au dixieme du prix affiche
              </p>

              <div id="calculateur" className="o-scroll-mt-24 o-mt-24 o-grid o-gap-10 lg:o-grid-cols-12">
                <div className="lg:o-col-span-4">
                  <Indice rang="03" sombre={false}>Le calculateur</Indice>
                  <h3 className="o-m-0 o-mt-5 o-text-zinc-950 dark:o-text-zinc-50" style={{ ...affiche('m', 800), fontSize: 'clamp(1.75rem, 3.2vw, 3.25rem)' }}>
                  ...
                  <p className="o-mt-5 o-max-w-sm o-text-sm o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">
                    Posez votre volume, dites la longueur de vos reponses, laissez le cache allume ou non. Prix hors taxes, grille ci-dessus.
                  </p>
                </div>
          ...
          <section id="cle" aria-labelledby="cle-titre" className="o-scroll-mt-24 o-relative o-isolate o-flex o-items-center o-justify-center o-overflow-hidden o-border-t o-border-black-10 dark:o-border-zinc-800 o-px-6 o-py-32 md:o-py-44">
            <Nappe couleurs={[accent(300), accent(100), 'var(--o-vitrine-seconde)']} opacite={0.5} />
            <div className="o-relative o-mx-auto o-grid o-w-full o-max-w-6xl o-items-center o-gap-12 lg:o-grid-cols-12">
              <div className="lg:o-col-span-6">
                <Indice rang="04" sombre={false}>Une cle</Indice>
                <h2 id="cle-titre" className="o-m-0 o-mt-5 o-text-zinc-950 dark:o-text-zinc-50" style={{ ...affiche('m', 800), fontSize: 'clamp(2rem, 4.5vw, 4.25rem)' }}>
                ...
                <p className="o-mt-5 o-max-w-md o-text-base o-leading-relaxed o-text-zinc-700 dark:o-text-zinc-300">
                  De quoi traiter environ deux millions de jetons sur halo-clair. Aucune carte demandee tant que le credit n est pas epuise.
                </p>
              </div>
              <div className="o-flex o-justify-center lg:o-col-span-6 lg:o-justify-end">
                <Flotte amplitude={9} duree={7} angle={-3}>
                  <GlassSurface colors={['--o-vitrine-400', '--o-palette-white']} blur={22} tint={0.18} thickness={1.3} className="o-w-full o-max-w-sm o-rounded-2xl o-p-6 o-text-zinc-950 dark:o-text-zinc-50">
                    <p className="o-m-0 o-flex o-items-center o-gap-2 o-font-mono o-text-xs o-uppercase o-tracking-widest">
                      <span aria-hidden="true" className="o-block o-size-2 o-rounded-full" style={{ backgroundColor: 'var(--o-palette-emerald-500)' }} />
                    ...
                    <a href="#essai" className="o-mt-6 o-inline-flex o-items-center o-gap-2 o-rounded-full o-px-5 o-py-2.5 o-text-sm o-font-semibold o-no-underline o-transition-transform hover:o-scale-105 focus:o-ring" style={aplat()}>
                      Obtenir une cle <Icon icon={ArrowRight} size={15} aria-hidden="true" />
                    </a>
                  </GlassSurface>
                </Flotte>
              </div>
        ...
        <footer className="o-border-t o-border-black-10 dark:o-border-zinc-800 o-px-6 o-pb-10 o-pt-16">
          <div className="o-mx-auto o-max-w-7xl">
            <dl className="o-m-0 o-grid o-gap-8 sm:o-grid-cols-3">
              {[
                { ville: 'Paris', fuseau: 'Europe/Paris', quoi: 'Modeles servis — region eu-par' },
                { ville: 'Francfort', fuseau: 'Europe/Berlin', quoi: 'Modeles servis — region eu-fra' },
                { ville: 'UTC', fuseau: 'UTC', quoi: 'Horodatage des journaux' },
              ].map((h) => (
                <div key={h.ville} className="o-border-t o-border-black-10 dark:o-border-zinc-800 o-pt-4">
                  <dt className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400">{h.quoi}</dt>
            ...
            <div className="o-mt-16 o-grid o-gap-8 o-font-mono o-text-xs o-uppercase o-leading-relaxed o-tracking-widest o-text-zinc-600 dark:o-text-zinc-400 md:o-grid-cols-12">
              <p className="o-m-0 o-flex o-items-start o-gap-2 md:o-col-span-4">
                <Icon icon={Sparkles} size={14} className="o-mt-0.5" style={{ color: ENCRE }} aria-hidden="true" />
                <span>Halo SAS<br />28 rue des Vinaigriers, 75010 Paris<br />bonjour@halo.example</span>
              </p>
              <ul className="o-m-0 o-flex o-list-none o-flex-wrap o-gap-x-6 o-gap-y-2 o-p-0 md:o-col-span-8 md:o-justify-end">
                {[
                  ...
                  <li key={mot}>
                    <a href={href} className="o-no-underline o-text-zinc-600 o-transition-colors hover:o-text-zinc-950 focus:o-ring dark:o-text-zinc-400 dark:hover:o-text-zinc-50">{mot}</a>
                  </li>
                ))}
              </ul>
            </div>
            <p className="o-m-0 o-mt-8 o-border-t o-border-black-10 dark:o-border-zinc-800 o-pt-5 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400">
              © 2026 Halo — plateforme fictive, aucun modele n est reellement appele depuis cette page
            </p>
          </div>
        </footer>
      </div>
