...
import { ArrowRight, ArrowUpRight, Check, Cog, Gauge, Weight } from '@odoro-cli/icons/filaire'
...
import { Accent, Actions, affiche, BarreGelule, Chiffres, Coin, Croix, Etiquette, Indice, Porte, Surgit, usePolices, verre } from './marche.jsx'
...
const ARTICLES: readonly (readonly [string, string])[] = [
  ['1.1', 'Le bras, son armoire de commande et son prehenseur, dimensionnes sur la piece du client.'],
  ['1.2', 'L analyse de risque et la mise en conformite machine au sens de la directive 2006/42/CE.'],
  ['1.3', 'Les protections : barrieres materielles, scrutateurs ou zones collaboratives selon le cas.'],
  ['1.4', 'La programmation, et son code source remis en clair, commente en francais, avec sa documentation.'],
  ['1.5', 'La formation de deux operateurs, sur site, pendant trois jours consecutifs.'],
  ['1.6', 'Douze mois de garantie piece et main-d oeuvre, astreinte 6 h — 22 h comprise.'],
]

/** Les conditions d execution, telles qu elles figurent au contrat. */
const CONDITIONS: readonly (readonly [string, string])[] = [
  ['Delai contractuel', 'Quatorze semaines : six d etude et d approvisionnement, quatre de montage, deux de reception a blanc a l atelier, deux de mise en service sur site. Penalites de retard au contrat.'],
  ['Propriete du programme', 'Le code source appartient au client des la reception. Il peut le faire modifier par un tiers sans consequence sur la garantie mecanique.'],
  ['Reception a blanc', 'La cellule tourne deux semaines a l atelier, sur les pieces reelles du client, avant expedition.'],
  ['Retrofit', 'Un robot de plus de dix ans peut etre repris — baie de commande, securite, prehenseur — pour 40 a 60 % du cout d une cellule neuve.'],
  ['Astreinte', 'Au-dela de la premiere annee : 4 200 EUR par an, reponse sous trente minutes de 6 h a 22 h du lundi au samedi, intervention sous vingt-quatre heures en Bretagne et Pays de la Loire.'],
]
...
const TEMOIGNAGES: readonly (readonly [string, string])[] = [
  ['Helene Berthaut — Fonderie Berthaut, Lorient', 'Le poste etait classe penible : quatorze tonnes soulevees par jour et par operateur. C est ce qui a decide la direction, avant meme le calcul de retour.'],
  ['Marc Le Gall — Conserverie de l Odet, Quimper', 'Deux operateurs redeployes au controle qualite, aucun licenciement. Cobalt a demande ce qu ils deviendraient avant de nous chiffrer quoi que ce soit.'],
  ['Sonia Vallet — Plasturgie Vallet, Vannes', 'Le taux de rebut sorti d atelier est passe de 2,1 a 0,4 % en quatre mois. Nous avions le chiffre avant la fin de la garantie.'],
  ['Tanguy Kerne — Biscuiterie Kerne, Douarnenez', 'Ils nous ont dit non une fois : notre piece etait trop souple pour un prehenseur a ventouses. Un integrateur qui refuse une affaire, on s en souvient.'],
]
...
function Ligne({ quoi, children }: { readonly quoi: string; readonly children: ReactElement | string }): ReactElement {
  return (
    <tr className="o-border-b o-border-zinc-300 dark:o-border-zinc-700">
      <th scope="row" className="o-w-48 o-py-3 o-pr-6 o-align-top o-text-left o-font-mono o-text-xs o-font-normal o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400 md:o-w-64">
        {quoi}
      </th>
      <td className="o-py-3 o-align-top o-text-sm o-leading-relaxed o-text-zinc-800 dark:o-text-zinc-200">{children}</td>
    </tr>
  )
}
...
export default function Page(): ReactElement {
  ...
  const calcul = useMemo(() => {
    const retenu = GAMME.find((b) => b.charge >= charge && b.portee >= portee) ?? GAMME[GAMME.length - 1]
  ...
  return (
    ...
    <div className="o-bg-white dark:o-bg-zinc-950 o-text-zinc-900 dark:o-text-zinc-100" style={polices}>
      ...
      <header id="haut" className="o-relative o-isolate o-min-h-screen o-overflow-hidden o-bg-zinc-50 dark:o-bg-zinc-950">
        <Halftone
        ...
        />
        <div aria-hidden="true" className="o-absolute o-inset-0 o-z-0" style={{ background: 'linear-gradient(to right, var(--o-theme-bg) 30%, color-mix(in oklab, var(--o-theme-bg) 55%, transparent) 55%, transparent 80%)' }} />

        <BarreGelule marque="Cobalt" liens={[['#preuve', 'Ce que change une cellule'], ['#abaque', 'L abaque'], ['#fiche', 'Fiche']]} action={['#abaque', 'Chiffrer une cellule']} sombre={false} />
        ...
        <div className="o-relative o-z-10 o-mx-auto o-grid o-min-h-screen o-max-w-7xl o-items-center o-gap-12 o-px-6 o-pb-64 o-pt-32 md:o-px-10 lg:o-grid-cols-12 lg:o-pb-48">
          ...
          <div className="o-min-w-0 lg:o-col-span-7">
            <Surgit>
              <Etiquette sombre={false}>Integrateur de robotique industrielle — Caudan</Etiquette>
            </Surgit>
            <Surgit delai={120} as="h1" className="o-m-0 o-mt-7 o-max-w-3xl o-text-zinc-950 dark:o-text-zinc-50" style={{ ...affiche('l', 300), fontSize: 'clamp(2.75rem, 6.4vw, 6.5rem)' }}>
              Un bras rembourse en <Accent couleur={encre()}>dix-neuf mois</Accent>, et nous l ecrivons avant de le vendre.
            </Surgit>
            <Surgit delai={420} as="p" className="o-m-0 o-mt-7 o-max-w-xl o-text-lg o-leading-relaxed o-text-zinc-600 dark:o-text-zinc-400">
              Un industriel n achete pas un robot, il achete un delai de remboursement. L abaque au milieu de cette page le calcule avec vos chiffres, pas les notres.
            </Surgit>
            <Surgit delai={540} className="o-mt-9">
              <Actions sombre={false} pleine={['#abaque', <>Chiffrer ma cellule <Icon icon={ArrowRight} size={16} aria-hidden="true" /></>]} fantome={['#preuve', 'Ce que change une cellule']} />
            </Surgit>
          </div>
        ...
        <Coin position="hd" sombre={false}>Fiche technique C-12<br />Edition 09.2026</Coin>
      </header>

      <main>
        {/* ================= 2. Le bandeau des secteurs, en mots geants ======= */}
        <div aria-label="Secteurs livres" className="o-border-b o-border-zinc-300 dark:o-border-zinc-700 o-py-6 o-text-zinc-950 dark:o-text-zinc-50" style={{ fontWeight: 300, letterSpacing: '-0.03em' }}>
          <Bandeau mots={SECTEURS} separateur="//" vitesse={70} taille="clamp(2.25rem, 6vw, 6rem)" />
        </div>

        {/* ================= 3. La preuve, en etapes defilantes ============== */}
        <section id="preuve" className="o-mx-auto o-max-w-6xl o-scroll-mt-24 o-px-6 o-py-20 md:o-py-28">
          ...
          <h2 className="o-m-0 o-mt-10 o-max-w-4xl o-text-zinc-950 dark:o-text-zinc-50" style={{ ...affiche('l', 300), fontSize: 'clamp(2.25rem, 6vw, 6rem)' }}>
            Ce qu une cellule change, poste par poste.
          </h2>
        ...
        <section id="abaque" className="o-scroll-mt-24 o-px-6 o-py-20 md:o-py-28" style={nuit('slate')}>
          <div className="o-mx-auto o-max-w-6xl">
            <Indice rang="02">L abaque</Indice>
            <h2 className="o-m-0 o-mt-5 o-max-w-2xl o-text-slate-50" style={{ ...affiche('m', 300), fontSize: 'clamp(2rem, 4.5vw, 4.25rem)' }}>
              Trois nombres, et le devis.
            </h2>
            <p className="o-m-0 o-mt-5 o-max-w-xl o-text-base o-leading-relaxed o-text-slate-400">
              Le prix affiche est celui de la cellule complete — bras, prehenseur, securite, integration, formation, garantie. Pas celui du seul robot.
            </p>
            ...
            <div className="o-mt-12 o-grid o-gap-10 lg:o-grid-cols-12 lg:o-gap-14">
              <div className="o-min-w-0 lg:o-col-span-7">
                <div className="o-flex o-flex-col o-gap-6">
                  {([
                    [Weight, 'Charge a manipuler, prehenseur compris', choix(CHARGES, charge, setCharge, 'kg')],
                    [Gauge, 'Portee necessaire', choix(PORTEES, portee, setPortee, 'mm')],
                    [Cog, 'Cadence visee, par heure', choix(CADENCES, cadence, setCadence, '/ h')],
                  ] as const).map(([icone, quoi, controle]) => (
                ...
                <div className="o-mt-10 o-overflow-x-auto">
                  <table className="o-w-full o-text-sm" style={{ minWidth: '34rem', borderCollapse: 'collapse' }}>
                    ...
                    <thead>
                      <tr className="o-border-b o-border-slate-600">
                        {([
                          ...
                          ['Cycle', 's'],
                          ['Cellule', 'EUR'],
                        ] as const).map(([entete, unite]) => (
                          <th key={entete} scope="col" className="o-py-2 o-pr-5 o-text-left o-font-mono o-text-xs o-font-normal o-uppercase o-tracking-wider o-text-slate-400">
                            {entete}
                            {unite !== '' && <span className="o-ml-1 o-lowercase">({unite})</span>}
                          </th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {GAMME.map((bras) => {
                        ...
                        return (
                          <tr key={bras.reference} className="o-border-b o-border-slate-800" style={retenu ? { backgroundColor: accentDoux(500, 18) } : undefined}>
                            <th scope="row" className="o-py-2.5 o-pl-2 o-pr-5 o-text-left o-font-mono o-font-bold" style={{ color: retenu ? encreSurSombre() : 'var(--o-palette-slate-50)' }}>
                              {bras.reference}
                              {retenu && <span className="o-sr-only"> — retenu</span>}
                            </th>
                            <td className="o-py-2.5 o-pr-5 o-font-mono o-tabular-nums o-text-slate-200">{bras.charge}</td>
                            <td className="o-py-2.5 o-pr-5 o-font-mono o-tabular-nums o-text-slate-200">{bras.portee}</td>
                            <td className="o-py-2.5 o-pr-5 o-font-mono o-tabular-nums o-text-slate-200">± {bras.repetabilite.toLocaleString('fr-FR')}</td>
                            <td className="o-py-2.5 o-pr-5 o-font-mono o-tabular-nums o-text-slate-200">{bras.cycle.toLocaleString('fr-FR')}</td>
                            <td className="o-py-2.5 o-font-mono o-tabular-nums o-text-slate-200">{bras.prix.toLocaleString('fr-FR')}</td>
                          </tr>
                        )
                      })}
                    </tbody>
                ...
                <h3 className="o-m-0 o-mt-12 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-slate-400">Ce que comprend une cellule</h3>
                <dl className="o-m-0 o-mt-4 o-grid o-gap-x-8 o-gap-y-2.5 md:o-grid-cols-2">
                  {ARTICLES.map(([rang, texte]) => (
                    <div key={rang} className="o-flex o-items-start o-gap-3 o-text-sm">
                      <dt className="o-shrink-0 o-font-mono o-text-xs o-tabular-nums" style={{ color: encreSurSombre() }}>{rang}</dt>
                      <dd className="o-m-0 o-leading-relaxed o-text-slate-300">{texte}</dd>
                    </div>
                  ))}
                </dl>
              </div>
              ...
              {calcul !== undefined && (
                <div className="lg:o-col-span-5">
                  <div className={`${verre(true)} o-relative o-p-7 lg:o-sticky`} style={{ top: 125 }}>
                    {/* Les croix aux coins du cadre — Fuel : le devis se lit
                        comme une piece jointe, pas comme une carte. */}
                    <Croix />
                    <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-slate-400">La cellule proposee</p>
                    <p className="o-m-0 o-mt-3 o-font-mono o-text-3xl o-font-bold o-tracking-tight" style={{ color: encreSurSombre() }}>
                      {calcul.bras > 1 ? `${String(calcul.bras)} × ` : ''}
                      {calcul.retenu.reference}
                    </p>
                    <p className="o-m-0 o-mt-2 o-text-sm o-leading-relaxed o-text-slate-400">{calcul.retenu.note}</p>

                    <dl aria-live="polite" className="o-m-0 o-mt-6 o-flex o-flex-col o-gap-2.5 o-border-t o-border-white-10 o-pt-5 o-text-sm">
                      {([
                        ['Cadence tenue', `${String(calcul.parHeure)} / h`],
                        ['Cellule complete', `${calcul.prix.toLocaleString('fr-FR')} EUR`],
                        ['Gain annuel estime', `${calcul.gain.toLocaleString('fr-FR')} EUR`],
                      ] as const).map(([quoi, valeur]) => (
                        <div key={quoi} className="o-flex o-items-baseline o-justify-between o-gap-4">
                          <dt className="o-text-slate-400">{quoi}</dt>
                          <dd className="o-m-0 o-font-mono o-tabular-nums o-font-medium o-text-slate-50">{valeur}</dd>
                        </div>
                      ))}
                    </dl>

                    <p className="o-m-0 o-mt-6 o-border-t o-border-white-10 o-pt-5">
                      <span className="o-block o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-slate-400">Retour sur investissement</span>
                      <span className="o-mt-1 o-flex o-items-baseline o-gap-3 o-text-slate-50">
                        <CountUp value={calcul.retour} duration={900} className="o-tabular-nums" style={{ ...affiche('l', 300), fontSize: 'clamp(3.5rem, 6vw, 6rem)' }} />
                        <span className="o-font-mono o-text-sm o-uppercase o-tracking-wider" style={{ color: encreSurSombre() }}>mois</span>
                      </span>
                    </p>

                    <p className="o-m-0 o-mt-4 o-text-xs o-leading-relaxed o-text-slate-400">
                      Hypotheses : un poste coute 46 000 EUR charges par an, et une cellule en libere 1,4 par bras — releve sur les 91 cellules livrees, non modelise.
                    </p>
        ...
        <section id="appel" aria-labelledby="appel-titre" className="o-scroll-mt-24 o-overflow-hidden o-py-10 md:o-py-14" style={aplat()}>
          <h2 id="appel-titre" className="o-sr-only">Nous ecrire</h2>
          <div aria-hidden="true" style={{ fontWeight: 300, letterSpacing: '-0.03em' }}>
            <Marquee speed={90} fade={0} pauseOnHover={false}>
              {APPEL.map((mot) => (
                <span key={mot} className="o-flex o-shrink-0 o-items-center o-whitespace-nowrap" style={{ fontSize: 'clamp(2.25rem, 6vw, 6rem)', lineHeight: 1 }}>
                  <span className="o-px-6">{mot}</span>
                  <span className="o-opacity-50" style={{ fontSize: '0.4em' }}>+</span>
                </span>
              ))}
            </Marquee>
          </div>
          <div className="o-mx-auto o-mt-8 o-flex o-max-w-6xl o-flex-wrap o-items-center o-justify-between o-gap-x-8 o-gap-y-3 o-px-6 o-font-mono o-text-xs o-uppercase o-tracking-widest md:o-px-10">
            <p className="o-m-0">Envoyez le plan et la cadence visee — reponse sous cinq jours ouvres</p>
            <a href="#haut" className="o-inline-flex o-items-center o-gap-2 o-text-sm o-normal-case o-tracking-normal o-underline o-underline-offset-4 focus:o-ring" style={{ color: 'inherit' }}>
              bureau@cobalt-robotique.fr <Icon icon={ArrowUpRight} size={14} aria-hidden="true" />
            </a>
          </div>
        </section>
      </main>
      ...
      <footer id="fiche" className="o-scroll-mt-24 o-px-6 o-pb-10 o-pt-16 md:o-px-10">
        <div className="o-mx-auto o-max-w-6xl">
          ...
          <div className="o-w-full o-text-zinc-950 dark:o-text-zinc-50" style={{ ...affiche('xxl', 700), fontSize: 'clamp(3rem, 22vw, 20rem)', lineHeight: 1, letterSpacing: '-0.02em' }}>
            <AsciiText rows={26} speed={4200} waves={0.8}>Cobalt</AsciiText>
          </div>
          <div className="o-mt-8 o-flex o-flex-wrap o-items-baseline o-justify-between o-gap-4">
            <p className="o-m-0 o-flex o-items-center o-gap-2 o-font-mono o-text-sm o-font-bold o-tracking-tight o-text-zinc-950 dark:o-text-zinc-50">
              <Icon icon={Cog} size={16} style={{ color: encre() }} aria-hidden="true" />
              Fiche de la societe
            </p>
            <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400">Edition 09.2026 — remplace 03.2026</p>
          </div>

          <table className="o-mt-6 o-w-full o-border-t o-border-zinc-900 dark:o-border-zinc-100" style={{ borderCollapse: 'collapse' }}>
            <caption className="o-sr-only">Fiche de la societe : coordonnees, conditions, references</caption>
            <tbody>
              <Ligne quoi="Raison sociale">Cobalt Robotique SAS au capital de 200 000 EUR — RCS Lorient 823 004 715</Ligne>
              <Ligne quoi="Siege et atelier">Zone de Kerpont, 56850 Caudan — vingt-deux personnes, dont neuf automaticiens</Ligne>
              <Ligne quoi="Zone d intervention">Bretagne et Pays de la Loire — astreinte 6 h a 22 h, du lundi au samedi</Ligne>
              <Ligne quoi="Applications">Mise en carton, palettisation, chargement de presse, degrappage et controle</Ligne>
              <Ligne quoi="Services">Etude et chiffrage, integration complete, retrofit de parc, astreinte</Ligne>
              <Ligne quoi="Documents">
                <span className="o-flex o-flex-wrap o-gap-x-6 o-gap-y-1">
                  {['Fiche technique gamme C', 'Conditions generales', 'Attestation d assurance', 'Declaration CE'].map((doc) => (
                    <a key={doc} href="#haut" className="o-inline-flex o-items-center o-gap-1 o-no-underline o-underline-offset-4 hover:o-underline focus:o-ring" style={{ color: encre() }}>
              ...
              {CONDITIONS.map(([quoi, texte]) => (
                <Ligne key={quoi} quoi={quoi}>{texte}</Ligne>
              ))}
              {RELEVE.map(([client, quoi, config, retour]) => (
                <Ligne key={client} quoi={client}>
                  <span className="o-flex o-flex-wrap o-items-baseline o-gap-x-4 o-gap-y-1">
                    <span>{quoi}</span>
                    <span className="o-font-mono o-text-xs o-text-zinc-500 dark:o-text-zinc-400">{config}</span>
                    <span className="o-inline-flex o-items-center o-gap-1 o-font-mono o-text-xs o-tabular-nums" style={{ color: encre() }}>
                      <Icon icon={Check} size={12} aria-hidden="true" /> retour en {retour}
                    </span>
                  </span>
                </Ligne>
              ))}
          ...
          <div className="o-mt-6 o-flex o-flex-wrap o-items-baseline o-justify-between o-gap-x-8 o-gap-y-2 o-font-mono o-text-xs o-text-zinc-500 dark:o-text-zinc-400">
            <p className="o-m-0">© 2026 Cobalt Robotique — les prix et retours affiches par l abaque sont indicatifs et confirmes par devis apres etude de la piece.</p>
            <a href="#haut" className="o-no-underline o-uppercase o-tracking-widest o-text-zinc-500 dark:o-text-zinc-400 hover:o-text-zinc-950 dark:hover:o-text-zinc-50 o-transition-colors focus:o-ring">Haut de page</a>
          </div>
        </div>
      </footer>
    </div>
