...
import { Activity, ArrowRight, ArrowUpRight, TriangleAlert } from '@odoro-cli/icons/filaire'
...
import { Actions, affiche, BarreGelule, Coin, Etiquette, Grain, Indice, Porte, Surgit, TitreVague, usePolices } from './marche.jsx'
...
const CONSTELLATION: readonly Sat[] = [
  { nom: 'APH-01', mission: 'Optique', etat: 'Nominal', altitude: 512, inclinaison: 97.4, resolution: '50 cm pan.', lance: 'Mars 2022', fenetre: '14 h 08 UTC', batterie: 97, stockage: 22, note: 'Premier de la serie Vireo. Reserve de propergol pour huit ans de maintien a poste.' },
  { nom: 'APH-02', mission: 'Optique', etat: 'Nominal', altitude: 512, inclinaison: 97.4, resolution: '50 cm pan.', lance: 'Mars 2022', fenetre: '15 h 41 UTC', batterie: 95, stockage: 61, note: 'Meme plan orbital que APH-01, dephase de 45 degres en anomalie moyenne.' },
  { nom: 'APH-05', mission: 'Radar', etat: 'Nominal', altitude: 570, inclinaison: 97.9, resolution: '1 m stripmap', lance: 'Sept. 2023', fenetre: '16 h 22 UTC', batterie: 91, stockage: 38, note: 'Bande X. Voit a travers la couverture nuageuse et de nuit, sans exception.' },
  { nom: 'APH-06', mission: 'Radar', etat: 'Degrade', altitude: 569, inclinaison: 97.9, resolution: '1 m stripmap', lance: 'Sept. 2023', fenetre: '17 h 55 UTC', batterie: 68, stockage: 44, note: 'Un des deux panneaux solaires ne se deploie qu a 80 %. Capacite reduite d un tiers, sans effet sur la qualite image.' },
  { nom: 'APH-08', mission: 'Infrarouge', etat: 'Nominal', altitude: 495, inclinaison: 97.2, resolution: '3,5 m therm.', lance: 'Juin 2024', fenetre: '06 h 12 UTC demain', batterie: 99, stockage: 12, note: 'Detection de foyers a partir de 4 metres carres. Employe par trois services de securite civile.' },
  { nom: 'APH-09', mission: 'Infrarouge', etat: 'Maintenance', altitude: 494, inclinaison: 97.2, resolution: '3,5 m therm.', lance: 'Juin 2024', fenetre: 'Reprise le 18 sept.', batterie: 88, stockage: 0, note: 'Recalage de la chaine de detection apres derive thermique. Immobilise onze jours, comme annonce le 2 septembre.' },
  { nom: 'APH-11', mission: 'Optique', etat: 'Nominal', altitude: 505, inclinaison: 97.4, resolution: '30 cm pan.', lance: 'Fev. 2026', fenetre: '13 h 30 UTC', batterie: 100, stockage: 8, note: 'Generation la plus recente. Trente centimetres au nadir, quarante a 30 degres de depointage.' },
  { nom: 'APH-12', mission: 'Optique', etat: 'Nominal', altitude: 505, inclinaison: 97.4, resolution: '30 cm pan.', lance: 'Fev. 2026', fenetre: '15 h 03 UTC', batterie: 100, stockage: 5, note: 'Jumeau de APH-11. Les deux ont ete lances sur le meme vol.' },
]

/** Le satellite lu par defaut, quand aucun choix ne tient. */
const SAT_ZERO: Sat = CONSTELLATION[6] ?? { nom: 'APH-11', mission: 'Optique', etat: 'Nominal', altitude: 505, inclinaison: 97.4, resolution: '30 cm pan.', lance: 'Fev. 2026', fenetre: '13 h 30 UTC', batterie: 100, stockage: 8, note: '' }
 ...
 */
const ACTE_ZERO = { id: 'allumage', etape: 'T − 0', titre: 'Allumage, Kourou.', texte: 'Ensemble de lancement ELV-4, 12 fevrier 2026, 09 h 20 UTC. Deux satellites de 410 kg sous la coiffe.', minutes: 0 } as const
...
const ACTES: readonly Acte[] = [
  ACTE_ZERO,
  { id: 'signal', etape: 'T + 9 min', titre: 'Le signal, depuis Kiruna.', texte: 'Separation a 505 km, puis acquisition du signal a la premiere visibilite. Panneaux deployes, batterie a 100 %.', minutes: 9, graine: 'aphelie-station', alt: 'Antenne de 7,3 m en bande X, station de Kiruna' },
  { id: 'image', etape: 'J + 3', titre: 'La premiere image.', texte: 'Parcelles du Beauce, trente centimetres au nadir. La chaine optique est calibree sur trois passages.', minutes: 4320, graine: 'aphelie-champ', alt: 'Parcelles agricoles, image satellite en fausses couleurs' },
  { id: 'service', etape: 'J + 30', titre: 'En service.', texte: 'Le port de Rotterdam, en veille continue depuis le 14 mars. Deux satellites de plus dans la table qui suit.', minutes: 43200, graine: 'aphelie-port', alt: 'Terminal a conteneurs vu du ciel' },
]
...
const FICHE: readonly (readonly [string, readonly string[]])[] = [
  ...
  ['Tarifs', ['Archive, 9 EUR par km2', 'Programmation, 28 EUR par km2', 'Veille continue, sur devis']],
  ['Documents', ['Specifications techniques', 'Conditions de licence', 'Rapport de revisite 2025']],
  ['Reglementation', ['Operateur declare, loi sur les operations spatiales', 'Reglement (UE) 2021/821, biens a double usage']],
          ...
          return (
            <span
              ...
              style={{
                ...
                width: taille,
                height: taille,
                boxShadow: gros > 0.93 ? '0 0 6px 1px rgba(255,255,255,0.55)' : undefined,
...
function ElevationCotee(): ReactElement {
  ...
  return (
    <svg viewBox="0 0 460 300" className="o-h-auto o-w-full" role="img" aria-label="Elevation cotee du satellite Vireo : envergure 6,40 metres, hauteur 2,15 metres, antenne de 0,90 metre">
      <g fill="none" stroke={trait} strokeWidth="1.2" strokeLinejoin="round" strokeLinecap="round">
        ...
        {cellules.map((decalage) => (
          <path key={`g${String(decalage)}`} d={`M${String(46 + decalage)} 104V164`} opacity="0.5" />
        ))}
        {cellules.map((decalage) => (
          <path key={`d${String(decalage)}`} d={`M${String(296 + decalage)} 104V164`} opacity="0.5" />
        ))}
      ...
      <g fill={cote} fontFamily="ui-monospace, monospace" fontSize="11" letterSpacing="0.08em">
        <text x="230" y="258" textAnchor="middle">6,40 m — envergure deployee</text>
        <text x="230" y="276" textAnchor="middle" opacity="0.75">Vireo, plateforme d observation — elevation, echelle 1:20</text>
        <text x="230" y="30" textAnchor="middle">0,90 m</text>
        <text x="424" y="136" textAnchor="end" transform="rotate(-90 424 136)">2,15 m</text>
      </g>
    </svg>
  )
}
 ...
 */
function Trace({ inclinaison, altitude, sombre = true }: { readonly inclinaison: number; readonly altitude: number; readonly sombre?: boolean }): ReactElement {
  const trait = sombre ? encreSurSombre() : encre()
  return (
    <div aria-hidden="true" className="o-relative o-aspect-square o-w-full o-max-w-xs">
      <div className="o-absolute o-rounded-full" style={{ inset: '33%', backgroundColor: 'color-mix(in oklab, var(--o-theme-fg) 10%, transparent)' }} />
      <div className="o-absolute o-rounded-full" style={{ inset: '33%', border: '1px solid var(--o-theme-line)' }} />
      {[16, -38].map((tour) => (
        <div key={tour} className="o-absolute o-inset-0 o-rounded-full" style={{ border: '1px solid var(--o-theme-line)', transform: `rotate(${String(tour)}deg) scaleX(0.4)` }} />
      ))}
      <div className="o-absolute o-inset-0 o-rounded-full" style={{ border: `1px solid ${trait}`, transform: `rotate(${String(inclinaison - 90)}deg) scaleX(0.4)` }} />
      <div
        className="o-absolute o-left-1/2 o-top-0 o-size-2 o-rounded-full"
        style={{ backgroundColor: trait, transform: `translate(-50%, -50%) rotate(${String(inclinaison - 90)}deg) translateY(${String(altitude / 14)}px)` }}
      />
    </div>
  )
}
...
function Jauge({ quoi, part }: { readonly quoi: string; readonly part: number }): ReactElement {
  return (
    <div>
      <div className="o-flex o-items-baseline o-justify-between o-gap-3">
        <span className="o-font-mono o-text-xs o-uppercase o-tracking-wider o-text-slate-400">{quoi}</span>
        <span className="o-font-mono o-text-sm o-tabular-nums" style={{ color: encreSurSombre() }}>{part} %</span>
      </div>
      <div aria-hidden="true" className="o-mt-2 o-h-1.5 o-w-full o-overflow-hidden o-rounded-full" style={{ backgroundColor: 'var(--o-theme-line)' }}>
        <div className="o-h-full o-rounded-full" style={{ width: `${String(part)}%`, backgroundColor: encreSurSombre() }} />
      </div>
    </div>
  )
}
...
function Compteur({ valeur, quoi }: { readonly valeur: number; readonly quoi: string }): ReactElement {
  return (
    <div className="o-border-l o-border-slate-200 dark:o-border-slate-800 o-pl-5 md:o-pl-8">
      <p className="o-m-0 o-font-mono o-tabular-nums o-text-slate-950 dark:o-text-slate-50" style={{ fontSize: 'clamp(3rem, 9vw, 9rem)', lineHeight: 1, fontWeight: 300, letterSpacing: '-0.04em' }}>
        <CounterRoll value={valeur} locale="fr-FR" duration={700} step={60} />
      </p>
      <p className="o-m-0 o-mt-3 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-slate-500 dark:o-text-slate-400">{quoi}</p>
    </div>
  )
}
...
function CompteARebours(): ReactElement {
  ...
  const s = secondes % 60
  return (
    <div className="o-grid o-grid-cols-2 o-gap-y-10 lg:o-grid-cols-4" role="timer" aria-label={`Prochain lancement dans ${String(jours)} jours, ${String(heures)} heures, ${String(minutes)} minutes`}>
...
export default function Page(): ReactElement {
  ...
  const lignes = useMemo(() => CONSTELLATION.filter((s) => mission === 'Toutes' || s.mission === mission), [mission])
  const sat: Sat = useMemo(() => CONSTELLATION.find((s) => s.nom === choisi) ?? SAT_ZERO, [choisi])
    ...
    () => (
      <Volume
        ...
        construire={(contexte) => {
          ...
          const matieres = [
            new three.MeshStandardMaterial({ color: alliage, metalness: 0.92, roughness: 0.3, flatShading: true }),
            new three.MeshStandardMaterial({ color: cellule, metalness: 0.55, roughness: 0.22 }),
            new three.MeshStandardMaterial({ color: alliage, metalness: 0.35, roughness: 0.5, side: three.DoubleSide }),
            new three.MeshStandardMaterial({ color: cuivre, metalness: 1, roughness: 0.42 }),
          ...
          const gAntenne = new three.SphereGeometry(0.6, 28, 14, 0, Math.PI * 2, 0, Math.PI / 3.2)
          ...
          const geometries = [gCorps, gAretes, gBras, gPanneau, gPlan, gGrille, gAntenne, gPerche, gCornet, gTuyere, gCol, gMat]
          ...
          vireo.add(new three.Mesh(gCorps, mAlliage), new three.LineSegments(gAretes, mFil))
          ...
          eclairer(contexte, { cle: 0xfff1dc, remplissage: 0x5f80c4, contour: 0xbcd6ff, force: 1.25 })
        ...
        animer={({ scene }, { delta, time }) => {
          ...
          panneaux.rotation.x = Math.atan2(Math.sin(lacet) * 0.47 + Math.cos(lacet) * 0.47, 0.66)
        }}
      />
    ),
    [],
  ...
  return (
    <Porte forme="compteur" marque="Aphelie">
      <div className="o-bg-white dark:o-bg-slate-950 o-text-slate-900 dark:o-text-slate-100" style={polices}>
        ...
        <header className="o-relative o-isolate o-min-h-screen o-overflow-hidden" style={nuit('slate')}>
          {engin}
          <Ciel />
          <Voile sens="gauche" famille="slate" />
          <Grain />

          <BarreGelule marque="Aphelie" liens={[['#sequence', 'Le lancement'], ['#constellation', 'La constellation'], ['#prochain', 'Prochain vol']]} action={['#constellation', 'Voir ce qui vole']} />
          ...
          <div className="o-relative o-z-10 o-mx-auto o-flex o-min-h-screen o-max-w-6xl o-flex-col o-justify-center o-px-6 o-pb-28 o-pt-32">
            <div className="o-max-w-2xl">
              <Surgit>
                <Etiquette>Douze satellites — {nominaux} sur {CONSTELLATION.length} nominaux</Etiquette>
              </Surgit>
              <TitreVague delai={120} className="o-m-0 o-mt-7 o-text-slate-50" style={{ ...affiche('l', 300), fontSize: 'clamp(2.5rem, 5.6vw, 5.75rem)' }}>
                Un point du globe, toutes les quatre heures.
              </TitreVague>
              <Surgit delai={420} as="p" className="o-m-0 o-mt-7 o-max-w-lg o-text-lg o-leading-relaxed o-text-slate-300">
                Optique a trente centimetres, radar bande X, infrarouge thermique. La revisite annoncee est celle mesuree.
              </Surgit>
              <Surgit delai={540} className="o-mt-9">
                <Actions pleine={['#constellation', <>Voir la constellation <Icon icon={ArrowRight} size={16} aria-hidden="true" /></>]} fantome={['#sequence', 'Le dernier lancement']} />
              </Surgit>
              <Surgit delai={660} as="p" className="o-m-0 o-mt-12 o-max-w-xs o-font-mono o-text-xs o-uppercase o-leading-relaxed o-tracking-widest o-text-slate-400">
                Plateforme Vireo, 410 kg — envergure 6,40 m panneaux deployes
              </Surgit>
            </div>
          </div>
          <Coin position="hg">Toulouse — Kiruna — Kourou — Hobart</Coin>
          <Coin position="hd">Releve 09.09.2026<br />12 h 40 UTC</Coin>
          <Coin position="bg">Vireo-4 en orbite depuis le 12.02.2026</Coin>
          <Coin position="bd">Prochain vol : Vireo-5<br />14.11.2026 — 09 h 20 UTC</Coin>
        </header>

        <main>
          {/* ================= (01) La sequence de lancement, epinglee : quatre actes ===== */}
          <section id="sequence" aria-label="La sequence de lancement de Vireo-4" className="o-scroll-mt-24">
            <Epingle ecrans={4} actes={ACTES.length}>
              {(acte, progression) => {
                const courant: Acte = ACTES[acte] ?? ACTE_ZERO
                return (
                  <div className="o-relative o-flex o-h-full o-flex-col o-overflow-hidden" style={nuit('slate')}>
                    {/* La ligne de progression, en haut de la scene. */}
                    <div aria-hidden="true" className="o-absolute o-inset-x-0 o-top-0 o-h-px o-bg-white-10">
                      <div className="o-h-full" style={{ width: `${String(Math.round(progression * 100))}%`, backgroundColor: encreSurSombre(), transition: 'width 600ms cubic-bezier(0.2, 0, 0, 1)' }} />
                    </div>
                    ...
                    <div className="o-mx-auto o-grid o-w-full o-max-w-6xl o-grow o-items-center o-gap-10 o-px-6 o-py-10 md:o-grid-cols-12">
                      <div className="o-min-w-0 md:o-col-span-6">
                        ...
                        <p className="o-m-0 o-mt-6 o-text-slate-50" style={{ ...affiche('xl', 300), fontSize: 'clamp(3rem, 9vw, 8rem)' }}>
                          <DepthText depth={10} step={2} angle={12} speed={8000} couleur={accent(700)}>
                            Vireo-4
                          </DepthText>
                        </p>
                        <div key={courant.id} className="o-mt-8">
                          <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest" style={{ color: encreSurSombre() }}>{courant.etape}</p>
                          <h2 className="o-m-0 o-mt-3 o-text-slate-50" style={{ ...affiche('m', 300), fontSize: 'clamp(1.75rem, 3.4vw, 3.25rem)' }}>{courant.titre}</h2>
                          <p className="o-m-0 o-mt-4 o-max-w-md o-text-base o-leading-relaxed o-text-slate-300">{courant.texte}</p>
                        </div>
                        <p className="o-m-0 o-mt-8 o-flex o-items-baseline o-gap-3 o-font-mono o-text-slate-50">
                          <span className="o-text-xs o-uppercase o-tracking-widest o-text-slate-400">Minutes apres l allumage</span>
                          <span className="o-text-3xl o-tabular-nums md:o-text-4xl">
                            <CounterRoll value={courant.minutes} locale="fr-FR" duration={800} step={70} />
                          </span>
                        </p>
                      </div>
                      ...
                      <div className="o-relative o-min-w-0 md:o-col-span-6">
                        {courant.graine === undefined ? (
                          ...
                          <figure key={courant.id} className="o-m-0">
                            <img src={photo(courant.graine, 1200, 800)} alt={courant.alt ?? ''} className="o-h-auto o-w-full o-rounded-2xl o-object-cover" style={{ aspectRatio: '4 / 3' }} />
                            <figcaption className="o-mt-3 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-slate-400">{courant.alt}</figcaption>
                          </figure>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="o-mx-auto o-w-full o-max-w-6xl o-px-6 o-pb-8" style={{ '--o-palette-brand-500': encreSurSombre() } as CSSProperties}>
                      <Stepper label="Etapes du lancement" value={acte} reach="none" steps={ACTES.map((a) => ({ id: a.id, label: a.etape, hint: a.titre }))} />
                    </div>
                  </div>
                )
              }}
          ...
          <section id="constellation" aria-labelledby="constellation-titre" className="o-scroll-mt-24 o-mx-auto o-max-w-6xl o-px-6 o-py-24 md:o-py-32">
            <div className="o-grid o-gap-8 md:o-grid-cols-12 md:o-items-end">
              <div className="md:o-col-span-8">
                <Indice rang="02" sombre={false}>La constellation — releve du 9 septembre, 12 h 40 UTC</Indice>
                <h2 id="constellation-titre" className="o-m-0 o-mt-6 o-text-slate-950 dark:o-text-slate-50" style={{ ...affiche('l', 300), fontSize: 'clamp(2.5rem, 6vw, 5.5rem)' }}>
              ...
              <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-leading-relaxed o-tracking-widest o-text-slate-500 dark:o-text-slate-400 md:o-col-span-4 md:o-text-right">
                Le degrade et celui en maintenance<br />y figurent, avec leur cause
              </p>
            </div>

            <div className="o-mt-12 o-grid o-gap-10 lg:o-grid-cols-12">
              <div className="o-min-w-0 lg:o-col-span-8">
                <div role="group" aria-label="Filtrer par mission" className="o-flex o-flex-wrap o-items-center o-gap-2 o-border-b o-border-slate-900 dark:o-border-slate-100 o-pb-3">
                ...
                <Crosshair coords speed={14} gap={20} color={encre()} className="o-relative">
                  <div className="o-overflow-x-auto">
                    <table className="o-w-full o-min-w-full o-text-left o-font-mono o-text-xs">
                      <caption className="o-sr-only">La constellation Aphelie : huit satellites, leur mission, leur etat et leur orbite</caption>
                      <thead>
                        <tr className="o-border-b o-border-slate-200 dark:o-border-slate-800 o-text-slate-500 dark:o-text-slate-400">
                          {['Engin', 'Mission', 'Etat', 'Alt. km', 'Incl.', 'Resolution', 'Lance', 'Prochain passage'].map((entete, index) => (
                            <th key={entete} scope="col" className={`o-py-3 o-pr-4 o-font-normal o-uppercase o-tracking-widest ${index === 3 || index === 4 ? 'o-text-right' : ''}`}>{entete}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {lignes.map((s) => {
                          ...
                          return (
                            <tr key={s.nom} className="o-border-b o-border-slate-200 dark:o-border-slate-800" style={actif ? { backgroundColor: accentDoux(500, 14) } : undefined}>
                              <th scope="row" className="o-py-3 o-pr-4 o-font-normal">
                                ...
                                >
                                  <span aria-hidden="true" className="o-inline-block o-size-2 o-shrink-0 o-rounded-full" style={{ backgroundColor: couleurEtat(s.etat, false) }} />
                                  {s.nom}
                                </button>
                              </th>
                              <td className="o-py-3 o-pr-4 o-text-slate-700 dark:o-text-slate-300">{s.mission}</td>
                              <td className="o-py-3 o-pr-4 o-uppercase o-tracking-wider" style={{ color: couleurEtat(s.etat, false) }}>{s.etat}</td>
                              <td className="o-py-3 o-pr-4 o-text-right o-tabular-nums o-text-slate-700 dark:o-text-slate-300">{s.altitude}</td>
                              <td className="o-py-3 o-pr-4 o-text-right o-tabular-nums o-text-slate-700 dark:o-text-slate-300">{s.inclinaison.toLocaleString('fr-FR')}°</td>
                              <td className="o-py-3 o-pr-4 o-whitespace-nowrap o-text-slate-700 dark:o-text-slate-300">{s.resolution}</td>
                              <td className="o-py-3 o-pr-4 o-whitespace-nowrap o-text-slate-700 dark:o-text-slate-300">{s.lance}</td>
                              <td className="o-py-3 o-whitespace-nowrap o-text-slate-700 dark:o-text-slate-300">{s.fenetre}</td>
                            </tr>
                          )
                        })}
                      </tbody>
                ...
                <p className="o-mt-3 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-slate-500 dark:o-text-slate-400">
                  {lignes.length} engins sur {CONSTELLATION.length} — {nominaux} nominaux, 1 degrade, 1 en maintenance
                </p>
              </div>

              {/* Le panneau de telemetrie, colle a droite : il suit la ligne choisie. */}
              <div className="lg:o-col-span-4">
                <div className="o-rounded-2xl o-p-6 lg:o-sticky" style={{ ...nuit('slate'), top: 132 }}>
                  <div className="o-flex o-flex-wrap o-items-center o-gap-x-4 o-gap-y-2 o-font-mono o-text-xs">
                    ...
                    <span className="o-flex o-items-center o-gap-1.5" style={{ color: encreSurSombre() }}>
                  ...
                  <h3 className="o-m-0 o-mt-2 o-font-mono o-text-3xl o-font-bold o-tracking-tight o-text-slate-50">{sat.nom}</h3>
                  <p className="o-m-0 o-mt-1 o-font-mono o-text-xs o-tabular-nums" style={{ color: encreSurSombre() }}>
                    i = {sat.inclinaison.toLocaleString('fr-FR')} deg — h = {sat.altitude} km
                  </p>
                  <p className="o-m-0 o-mt-4 o-text-sm o-leading-relaxed o-text-slate-300">{sat.note}</p>
                  ...
                  <ul className="o-m-0 o-mt-5 o-list-none o-border-t o-border-slate-800 o-p-0 o-pt-4">
                    <li className="o-py-1 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-slate-400">Passages a venir — UTC</li>
                    {PASSAGES.map(([heure, engin, station, quoi]) => (
                      <li key={`${heure}-${engin}`} className="o-flex o-flex-wrap o-items-baseline o-gap-x-3 o-py-1 o-font-mono o-text-xs">
                        <span className="o-tabular-nums o-font-semibold" style={{ color: engin === sat.nom ? encreSurSombre() : 'var(--o-palette-slate-50)' }}>{heure}</span>
                        <span className={engin === sat.nom ? 'o-text-slate-50' : 'o-text-slate-400'}>{engin}</span>
                        <span className="o-text-slate-400">{station}</span>
                        <span className="o-w-full o-text-slate-400 xl:o-w-auto">{quoi}</span>
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <section id="prochain" aria-labelledby="prochain-titre" className="o-scroll-mt-24 o-border-t o-border-slate-200 dark:o-border-slate-800 o-px-6 o-py-24 md:o-py-36">
            <div className="o-mx-auto o-max-w-6xl">
              <Reveal>
                <Indice rang="03" sombre={false}>Prochain lancement</Indice>
                <h2 id="prochain-titre" className="o-m-0 o-mt-6 o-font-mono o-text-sm o-uppercase o-tracking-widest o-text-slate-950 dark:o-text-slate-50">
              ...
              <Reveal delay={200} className="o-mt-14">
                <a href="#constellation" className="o-inline-flex o-items-center o-gap-2 o-font-mono o-text-xs o-uppercase o-tracking-widest o-no-underline focus:o-ring" style={{ color: encre() }}>
                  Recevoir l heure exacte de la fenetre <Icon icon={ArrowUpRight} size={14} aria-hidden="true" />
                </a>
              </Reveal>
            </div>
          </section>
        ...
        <footer className="o-border-t o-border-slate-900 dark:o-border-slate-100 o-px-6 o-pb-8 o-pt-10">
          <div className="o-mx-auto o-max-w-6xl">
            <table className="o-w-full o-text-left o-text-sm" style={{ borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {FICHE.map(([quoi, valeurs]) => (
                  <tr key={quoi} className="o-border-t o-border-slate-200 dark:o-border-slate-800">
                    <th scope="row" className="o-w-40 o-py-3 o-pr-6 o-align-top o-font-mono o-text-xs o-font-normal o-uppercase o-tracking-widest o-text-slate-500 dark:o-text-slate-400 md:o-w-56">{quoi}</th>
                    <td className="o-py-3">
                      <ul className="o-m-0 o-flex o-list-none o-flex-wrap o-gap-x-8 o-gap-y-1 o-p-0">
                        {valeurs.map((v) => (
                          <li key={v}>
                            {quoi === 'Documents' || quoi === 'Contact' ? (
                              <a href="#constellation" className="o-no-underline o-text-slate-800 dark:o-text-slate-200 hover:o-text-slate-950 dark:hover:o-text-slate-50 focus:o-ring">{v} ↗</a>
                            ) : (
                              <span className="o-text-slate-800 dark:o-text-slate-200">{v}</span>
                            )}
                          </li>
                        ))}
                      </ul>
            ...
            <p className="o-m-0 o-mt-6 o-border-t o-border-slate-900 dark:o-border-slate-100 o-pt-4 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-slate-500 dark:o-text-slate-400">
              © 2026 Aphelie — les acquisitions a 30 cm demandent, sur certaines zones, une autorisation prealable obtenue en huit a vingt jours ouvres.
            </p>
          </div>
        </footer>
      </div>
