...
const CATALOGUE: readonly Astre[] = [
  { nom: 'Vega', genre: 'Etoile — Lyre', ad: 18.615, dec: 38.78, magnitude: 0.03, mot: 'Le zero de l echelle photometrique, par convention.' },
  { nom: 'Deneb', genre: 'Etoile — Cygne', ad: 20.69, dec: 45.28, magnitude: 1.25, mot: 'A deux mille annees-lumiere : l etoile la plus lointaine que l oeil nu atteigne.' },
  { nom: 'Altair', genre: 'Etoile — Aigle', ad: 19.846, dec: 8.87, magnitude: 0.77, mot: 'Elle tourne en neuf heures et s en trouve aplatie d un cinquieme.' },
  { nom: 'Arcturus', genre: 'Etoile — Bouvier', ad: 14.261, dec: 19.18, magnitude: -0.05, mot: 'Une geante orange qui traverse le disque galactique par le travers.' },
  { nom: 'Capella', genre: 'Etoile — Cocher', ad: 5.278, dec: 45.998, magnitude: 0.08, mot: 'Quatre etoiles, dont deux geantes jaunes que rien ne separe a l oeil.' },
  { nom: 'Polaris', genre: 'Etoile — Petite Ourse', ad: 2.53, dec: 89.26, magnitude: 1.98, mot: 'A trois quarts de degre du pole : elle decrit un petit cercle, elle ne l occupe pas.' },
  { nom: 'Betelgeuse', genre: 'Etoile — Orion', ad: 5.919, dec: 7.407, magnitude: 0.45, mot: 'Une supergeante variable dont le diametre depasse l orbite de Mars.' },
  { nom: 'Rigel', genre: 'Etoile — Orion', ad: 5.242, dec: -8.2, magnitude: 0.18, mot: 'Bleue, chaude, et cent vingt mille fois plus lumineuse que le Soleil.' },
  { nom: 'Sirius', genre: 'Etoile — Grand Chien', ad: 6.752, dec: -16.716, magnitude: -1.46, mot: 'La plus brillante du ciel, et la plus proche que l on voie de nos latitudes.' },
  { nom: 'Mizar', genre: 'Etoile double — Grande Ourse', ad: 13.399, dec: 54.93, magnitude: 2.23, mot: 'Le test de vue des archers : qui separe Alcor a l oeil nu voit bien.' },
  { nom: 'Albireo', genre: 'Etoile double — Cygne', ad: 19.512, dec: 27.96, magnitude: 3.05, mot: 'Or et bleu dans le meme champ. Le plus beau couple du ciel d ete.' },
  { nom: 'M31 — Andromede', genre: 'Galaxie', ad: 0.712, dec: 41.27, magnitude: 3.44, mot: 'Deux millions et demi d annees-lumiere, et six fois la largeur de la Lune.' },
  { nom: 'M13 — Hercule', genre: 'Amas globulaire', ad: 16.695, dec: 36.46, magnitude: 5.8, mot: 'Trois cent mille etoiles dans vingt annees-lumiere de diametre.' },
  { nom: 'M57 — l Anneau', genre: 'Nebuleuse planetaire', ad: 18.893, dec: 33.03, magnitude: 8.8, mot: 'Une etoile morte vue par le trou : un rond de fumee de mille ans.' },
  { nom: 'M42 — Orion', genre: 'Nebuleuse diffuse', ad: 5.588, dec: -5.39, magnitude: 4, mot: 'La pouponniere la plus proche : mille etoiles y naissent encore.' },
]
...
function viser(astre: Astre, site: Site, tsl: number): { hauteur: number; azimut: number } {
  ...
  const azimut =
    ((Math.atan2(-Math.cos(d) * Math.sin(h), Math.sin(d) * Math.cos(p) - Math.cos(d) * Math.sin(p) * Math.cos(h)) /
      RADIAN) %
      360 +
      360) %
    360
 ...
 */
function Coupole({ ouverture = 1, taille = 420 }: { readonly ouverture?: number; readonly taille?: number }): ReactElement {
  const large = 22 + ouverture * 34
  return (
    <svg viewBox="0 0 400 300" width={taille} className="o-h-auto o-w-full" aria-hidden="true">
      {/* Le tambour de beton. */}
      <path d="M96 300V188h208v112Z" fill={accentDoux(900, 62)} stroke={accentDoux(200, 52)} strokeWidth="1.5" />
      <path d="M96 220h208M96 252h208" stroke={accentDoux(200, 24)} strokeWidth="1" />
      <path d="M176 300v-52h48v52" fill={accentDoux(950, 70)} stroke={accentDoux(200, 34)} strokeWidth="1" />

      {/* La calotte. */}
      <path d="M88 190a112 106 0 0 1 224 0Z" fill={accentDoux(800, 46)} stroke={accentDoux(200, 62)} strokeWidth="1.5" />
      <path d="M124 190a76 72 0 0 1 152 0" fill="none" stroke={accentDoux(200, 26)} strokeWidth="1" />
...
const ACTES = [
  { heure: '21 h 40', titre: 'La coupole s ouvre', texte: 'Le tambour tourne de quatre-vingt-dix degres. Il faut vingt minutes pour que l air du dedans prenne la temperature du dehors.' },
  { heure: '22 h 30', titre: 'La nuit astronomique', texte: 'Le Soleil passe sous dix-huit degres. Avant cela, le fond du ciel est encore eclaire, et les nebuleuses ne sortent pas.' },
  { heure: '00 h 15', titre: 'Le meridien', texte: 'Les objets d ete passent au plus haut. C est la que l air en travers est le plus mince — une masse d air contre deux a trente degres.' },
  { heure: '02 h 50', titre: 'La rosee', texte: 'Le miroir descend sous le point de rosee. La resistance chauffante du secondaire tient le verre a deux degres au-dessus.' },
  { heure: '05 h 20', titre: 'On referme', texte: 'L aube nautique. Le tambour revient au nord, la fente se ferme, et le releve de la nuit part au catalogue.' },
] as const

/** Un semis d etoiles fixe, pour les couches dessinees du diorama. */
function semis(nombre: number, graine: number): readonly { x: number; y: number; r: number }[] {
  let etat = graine
...
function Voute(): ReactElement {
  ...
  return (
    <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="o-h-full o-w-full" aria-hidden="true">
      <g
        style={{
      ...
      >
        ...
        {etoiles.map((point, rang) => (
          <circle key={rang} cx={point.x} cy={point.y} r={point.r * 0.55} fill={accent(50)} opacity={0.35 + point.r * 0.65} />
        ))}
      </g>
    </svg>
  )
...
function Crete({ opacite, hauteur }: { readonly opacite: number; readonly hauteur: number }): ReactElement {
  return (
    <svg viewBox="0 0 1200 200" preserveAspectRatio="none" className="o-h-full o-w-full" aria-hidden="true">
      <path
 ...
 */
function CielDeCeSoir({ site, poserSite }: { readonly site: Site; readonly poserSite: (cle: string) => void }): ReactElement {
  ...
  const releve = useMemo(
    () =>
      CATALOGUE.map((astre) => ({ astre, ...viser(astre, site, tsl) })).sort((a, b) => b.hauteur - a.hauteur),
    [site, tsl],
  )
    ...
    <div className="o-grid o-gap-10 lg:o-grid-cols-12 lg:o-gap-14">
      ...
      <div className="o-min-w-0 lg:o-col-span-4">
        <fieldset className="o-m-0 o-p-0">
          <legend className="o-mb-4 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-400">Le lieu</legend>
          ...
          <p className="o-m-0 o-mt-4 o-font-mono o-text-xs o-tabular-nums o-text-zinc-400">
            {Math.abs(site.latitude).toFixed(2).replace('.', ',')}° {site.latitude >= 0 ? 'N' : 'S'} —{' '}
            {Math.abs(site.longitude).toFixed(2).replace('.', ',')}° {site.longitude >= 0 ? 'E' : 'O'} — {site.altitude} m
          </p>
        </fieldset>
        ...
        <div className="o-mt-10">
          <p className="o-m-0 o-flex o-flex-wrap o-items-baseline o-justify-between o-gap-3 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-400">
            L heure
            <span className="o-tabular-nums o-text-zinc-50">{maintenant ? format.format(instant) : `${deux(heure)} h 00`}</span>
          </p>
        ...
        <dl className="o-m-0 o-mt-10">
          {(
            [
              ['Temps sideral local', `${deux(tsl / 15)} h ${deux(((tsl / 15) % 1) * 60)}`],
              ['Objets au-dessus de l horizon', `${String(visibles.length)} sur ${String(CATALOGUE.length)}`],
              ['Nuits claires par an', String(site.nuitsClaires)],
            ] as const
          ).map(([quoi, valeur]) => (
            <div key={quoi} className="o-flex o-flex-wrap o-items-baseline o-justify-between o-gap-3 o-border-t o-border-white-10 o-py-3">
      ...
      <div className="o-min-w-0 lg:o-col-span-8">
        ...
        <div className="o-relative o-overflow-x-auto" style={{ overflowY: 'hidden' }}>
          <table className="o-w-full o-text-left" style={{ borderCollapse: 'collapse', minWidth: 560 }}>
            ...
            <thead>
              <tr className="o-border-b o-border-white-10">
                {['Objet', 'Genre', 'Hauteur', 'Direction', 'Magnitude'].map((mot) => (
                  <th key={mot} scope="col" className="o-py-3 o-pr-4 o-font-mono o-text-xs o-font-normal o-uppercase o-tracking-widest o-text-zinc-400">
                    {mot}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {releve.map(({ astre, hauteur, azimut }) => {
                ...
                return (
                  <tr key={astre.nom} className="o-border-b o-border-white-10" style={{ opacity: dessus ? 1 : 0.4 }}>
                    <th scope="row" className="o-py-3 o-pr-4 o-text-sm o-font-medium o-text-zinc-50">
                      {astre.nom}
                    </th>
                    <td className="o-py-3 o-pr-4 o-text-sm o-text-zinc-400">{astre.genre}</td>
                    <td className="o-py-3 o-pr-4 o-font-mono o-text-sm o-tabular-nums" style={{ color: dessus ? encreSurSombre() : 'var(--o-theme-muted)' }}>
        ...
        <p aria-live="polite" className="o-m-0 o-mt-6 o-max-w-2xl o-text-sm o-leading-relaxed o-text-zinc-400">
...
function EchelleDuNoir({ site }: { readonly site: Site }): ReactElement {
  ...
  return (
    <div className="o-relative o-flex o-items-stretch o-gap-8">
      <svg viewBox="0 0 130 470" className="o-h-auto" style={{ width: 130 }} role="img" aria-label={`Noir du ciel a ${site.nom} : ${site.noir.toFixed(1)} magnitudes par seconde d arc au carre`}>
        {/* La reglette. */}
        ...
        {SITES.filter((autre) => autre.cle !== site.cle).map((autre) => (
          <path key={autre.cle} d={`M44 ${enY(autre.noir).toFixed(1)}h64`} stroke={accentDoux(300, 26)} strokeWidth="1" strokeDasharray="3 4" />
        ))}

        {/* La valeur, posee dessus. */}
        <g>
          <path d={`M30 ${enY(site.noir).toFixed(1)}h78`} stroke={encreSurSombre()} strokeWidth="2.5" />
      ...
      <div className="o-relative o-min-w-0 o-grow">
        <div
          ...
          style={{ top: `${String(((enY(site.noir) - 24) / 470) * 100)}%`, transform: 'translateY(-50%)' }}
        >
          <p className="o-m-0 o-tabular-nums o-text-zinc-50" style={{ ...affiche('m', 300), fontSize: 'clamp(2.5rem, 6vw, 4.5rem)', lineHeight: 0.9 }}>
            {site.noir.toFixed(1).replace('.', ',')}
          </p>
          <p className="o-m-0 o-mt-2 o-font-mono o-text-xs o-uppercase o-tracking-widest" style={{ color: encreSurSombre() }}>
          ...
          <p className="o-m-0 o-mt-4 o-max-w-sm o-text-sm o-leading-relaxed o-text-zinc-400">
            Un ciel de grande ville tombe a dix-sept virgule cinq : on y voit deux cents etoiles. Ici, on en
            compte plus de deux mille, et la Voie lactee porte une ombre au sol.
          </p>
        </div>
      </div>
    </div>
...
function CompteARebours(): ReactElement {
  const [reste, setReste] = useState<{ j: number; h: number; m: number; s: number } | null>(null)
  ...
  const dateNuit =
    quand === null
      ? ''
      : new Intl.DateTimeFormat('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' }).format(quand.nuit)
  ...
  return (
    <div className="o-text-center">
      ...
      <div className="o-mt-6 o-flex o-flex-wrap o-items-baseline o-justify-center o-gap-x-6 o-gap-y-3" aria-live="off">
        {(
          [
            ['jours', reste?.j ?? 0],
            ['heures', reste?.h ?? 0],
          ...
          <span key={mot} className="o-inline-flex o-flex-col o-items-center">
            <span className="o-tabular-nums o-text-zinc-50" style={{ ...affiche('l', 300), fontSize: 'clamp(3rem, 9vw, 7rem)', lineHeight: 0.86 }}>
              {deux(valeur)}
            </span>
            <span className="o-mt-2 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-400">{mot}</span>
          </span>
        ))}
      </div>
      <p className="o-m-0 o-mt-8 o-text-base o-leading-relaxed o-text-zinc-300">
        Prochaine nuit publique — {dateNuit}, 21 h 30, sous la coupole. Douze places, pas une de plus : c est
        le nombre de personnes qui tiennent autour de l oculaire sans se marcher dessus.
      </p>
...
function CarteDuMonde({ site }: { readonly site: Site }): ReactElement {
  const points = useMemo(() => {
    ...
    for (let x = 4; x < 96; x += 1.8) {
      for (let y = 8; y < 84; y += 3.2) {
        if (TERRES.some(([tx, ty, tl, th]) => x >= tx && x <= tx + tl && y >= ty && y <= ty + th)) {
          liste.push({ x, y })
        }
      }
    }
  ...
  return (
    <div className="o-relative">
      <svg viewBox="0 0 100 92" className="o-h-auto o-w-full" role="img" aria-label="Carte du monde a points : quatre sites d observation allumes">
        {points.map((point, rang) => (
          <circle key={rang} cx={point.x} cy={point.y} r="0.62" fill={accentDoux(300, 42)} />
        ))}
        {SITES.map((autre) => {
          const actif = autre.cle === site.cle
          return (
            <g key={autre.cle}>
              <circle cx={autre.carte[0]} cy={autre.carte[1]} r={actif ? 2.4 : 1.5} fill={actif ? encreSurSombre() : accent(400)} />
              {actif && <circle cx={autre.carte[0]} cy={autre.carte[1]} r="5" fill="none" stroke={encreSurSombre()} strokeWidth="0.5" />}
              <text
                x={autre.carte[0] + (autre.cle === 'aubrac' ? -4 : 4)}
                y={autre.carte[1] + (autre.cle === 'mejean' ? -3.4 : autre.cle === 'aubrac' ? 4.6 : 1.4)}
                fontSize="2.6"
...
const PROGRAMME: readonly OrbitalStep[] = [
  { id: 'accueil', icon: <Pastille d="M4 20V10l8-6 8 6v10M9 20v-6h6v6" />, title: 'Accueil', date: '21 h 30', status: 'done' as const, energy: 100, content: 'Sous le tambour, lampe rouge obligatoire. La vision de nuit met vingt minutes a s installer et trois secondes a se perdre.' },
  { id: 'oeil', icon: <Pastille d="M2 12s3.6-6 10-6 10 6 10 6-3.6 6-10 6-10-6-10-6ZM12 9.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5Z" />, title: 'A l oeil nu', date: '22 h 00', status: 'done' as const, energy: 82, content: 'Reperage des constellations de saison, de la Voie lactee et de la lumiere zodiacale quand elle sort.' },
  { id: 'oculaire', icon: <Pastille d="M9 3h6v5l4 11a2 2 0 0 1-2 3H7a2 2 0 0 1-2-3l4-11ZM9 8h6" />, title: 'A l oculaire', date: '22 h 45', status: 'current' as const, energy: 64, content: 'Le trois cents millimetres ouvert a quatre. Amas, nebuleuses, et la Lune seulement si elle est mince.' },
  { id: 'pose', icon: <Pastille d="M4 18V8a2 2 0 0 1 2-2h2l1.4-2h5.2L16 6h2a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2ZM12 9a4 4 0 1 0 0 8 4 4 0 0 0 0-8Z" />, title: 'La pose longue', date: '00 h 30', status: 'todo' as const, energy: 46, content: 'Une heure sur une seule cible, guidee. On repart avec le fichier brut et la carte du champ.' },
  { id: 'releve', icon: <Pastille d="M6 3h9l3 3v15H6ZM9 10h6M9 14h6M9 18h4" />, title: 'Le releve', date: '05 h 20', status: 'todo' as const, energy: 28, content: 'Fermeture de la fente, releve des conditions, envoi au catalogue. Cafe sous le tambour.' },
]
    ...
    <Porte forme="trou" marque="Coupole">
      <div className="o-relative o-text-zinc-50" style={{ ...polices, ...nuit('zinc') }}>
        ...
        <header className="o-relative o-isolate o-flex o-flex-col o-overflow-hidden" style={{ minHeight: ECRAN }}>
          ...
          />
          {!reduced && <Meteors count={9} angle={210} color={accent(200)} className="o-absolute o-inset-0 o-z-0" />}
          ...
          <div className="o-relative o-z-10 o-flex o-grow o-flex-col o-justify-end o-px-6 o-pb-10 o-pt-10 md:o-px-12">
            <div className="o-grid o-items-end o-gap-10 lg:o-grid-cols-12">
              <div className="o-min-w-0 lg:o-col-span-7">
                <Surgit>
                  <Etiquette>Observatoire du causse — ouvert au public le vendredi</Etiquette>
                </Surgit>
                <TitreVague
                  delai={140}
                  className="o-m-0 o-mt-6 o-max-w-3xl"
                  style={{ ...affiche('l', 300), fontSize: 'clamp(2.6rem, 7.4vw, 7.5rem)', lineHeight: 0.88 }}
                >
                  Le ciel de ce soir, a votre heure et a votre latitude.
                </TitreVague>
                <Surgit delai={540} as="p" className="o-m-0 o-mt-7 o-max-w-md o-text-base o-leading-relaxed o-text-zinc-300">
                  Un trois cents millimetres sous une coupole de quatre metres, a mille vingt metres, sous un
                  ciel a vingt et une virgule six. Cette page calcule ce qui est au-dessus de l horizon quand
                  vous la lisez.
                </Surgit>
                <Surgit delai={660} className="o-mt-9 o-flex o-flex-wrap o-items-center o-gap-4">
                  <a
                    ...
                    style={aplat()}
                  >
                    Ouvrir le releve <Icon icon={ArrowDown} size={15} aria-hidden="true" />
                  </a>
                  <a
                    href="#venir"
              ...
              <div className="o-relative o-min-w-0 lg:o-col-span-5">
                <Surgit delai={380}>
                  <div className="o-relative o-mx-auto o-flex o-items-end o-justify-center" style={{ maxWidth: 440 }}>
                    <Coupole ouverture={0.85} />
                    <span className="o-pointer-events-none o-absolute o-left-1/2 o-top-0" style={{ transform: 'translate(-50%, -46%)', color: accent(200) }}>
        ...
        <main>
          ...
          <section id="nuit" className="o-scroll-mt-24">
                ...
                return (
                  <div className="o-flex o-h-full o-flex-col o-justify-between o-px-6 o-py-10 md:o-px-12">
                    <div className="o-flex o-items-start o-justify-between o-gap-6">
                      <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-text-zinc-400">
                        La nuit du vendredi — {String(acte + 1).padStart(2, '0')} / {String(ACTES.length).padStart(2, '0')}
                      </p>
                    ...
                    <div className="o-max-w-xl">
                      <h2 className="o-m-0 o-text-balance" style={{ ...affiche('m', 300), fontSize: 'clamp(1.9rem, 5vw, 4.25rem)', lineHeight: 0.95 }}>
                        {a.titre}
                      </h2>
                      <p className="o-m-0 o-mt-5 o-max-w-lg o-text-base o-leading-relaxed o-text-zinc-300">{a.texte}</p>
                    </div>
                  </div>
                )
              }}
            >
              ...
              <Couche profondeur={0} className="o-bg-zinc-950">
                <div className="o-absolute o-inset-0" style={{ background: `radial-gradient(70% 52% at 50% 4%, ${accentDoux(800, 40)}, transparent 76%)` }} />
              </Couche>
              ...
              <Couche profondeur={0.24} derive={22} className="o-opacity-40">
                ...
                <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="o-h-full o-w-full" aria-hidden="true">
                  {[26, 44, 62].map((r) => (
                    <ellipse key={r} cx="50" cy="88" rx={r} ry={r * 0.74} fill="none" stroke={accentDoux(300, 26)} strokeWidth="0.2" strokeDasharray="1 2" />
                  ))}
                  <path d="M50 12V88M12 88h76" stroke={accentDoux(300, 20)} strokeWidth="0.2" />
                </svg>
              </Couche>
              <Couche profondeur={0.06} derive={26} className="o-flex o-items-end o-justify-center">
          ...
          <section id="ciel" className="o-scroll-mt-24 o-border-t o-border-white-10 o-px-6 o-py-24 md:o-px-12 md:o-py-32">
            <div className="o-mx-auto o-max-w-7xl">
              <div className="o-grid o-gap-8 md:o-grid-cols-12 md:o-items-end">
                <div className="o-min-w-0 md:o-col-span-8">
                  ...
                  <Reveal delay={80}>
                    <h2 className="o-m-0 o-mt-5 o-max-w-3xl o-text-balance" style={{ ...affiche('m', 300), fontSize: 'clamp(1.9rem, 4.6vw, 4.25rem)', lineHeight: 0.95 }}>
          ...
          {/* ================= L echelle du noir ========================== */}
          <section className="o-border-t o-border-white-10 o-px-6 o-py-24 md:o-px-12 md:o-py-32" style={{ backgroundColor: accentDoux(500, 6) }}>
              ...
              <div className="o-min-w-0 lg:o-col-span-5">
                ...
                <Reveal delay={80}>
                  <h2 className="o-m-0 o-mt-5 o-max-w-md o-text-balance" style={{ ...affiche('m', 300), fontSize: 'clamp(1.8rem, 4vw, 3.5rem)', lineHeight: 0.96 }}>
                    Ce qu on vient chercher ici n est pas un instrument, c est une absence.
                  </h2>
                </Reveal>
                <p className="o-m-0 o-mt-6 o-max-w-md o-text-base o-leading-relaxed o-text-zinc-300">
                  Un telescope deux fois plus grand gagne une magnitude et demie. Passer d un ciel de
                  peripherie a celui-ci en gagne trois. Le site compte plus que le verre, et c est pour cela
                  que la coupole est la et pas ailleurs.
                </p>
              </div>
              <div className="o-min-w-0 lg:o-col-span-7">
          ...
          <section className="o-border-t o-border-white-10 o-px-6 o-py-24 md:o-px-12 md:o-py-32">
            <div className="o-mx-auto o-max-w-7xl">
              ...
              <div className="o-mt-12 o-grid o-gap-12 lg:o-grid-cols-12 lg:o-gap-16">
                <div className="o-min-w-0 lg:o-col-span-5">
                  <Reveal delay={80}>
                    <h2 className="o-m-0 o-max-w-md o-text-balance" style={{ ...affiche('m', 300), fontSize: 'clamp(1.8rem, 4vw, 3.5rem)', lineHeight: 0.96 }}>
                  ...
                  <ol className="o-m-0 o-mt-10 o-list-none o-p-0">
                    {PROGRAMME.map((etape) => (
                      <li key={etape.id} className="o-grid o-gap-3 o-border-t o-border-white-10 o-py-5 sm:o-grid-cols-12">
                        <p className="o-m-0 o-font-mono o-text-xs o-uppercase o-tracking-widest o-tabular-nums sm:o-col-span-3" style={{ color: encreSurSombre() }}>
                          {etape.date}
                        </p>
                        <div className="o-min-w-0 sm:o-col-span-9">
                          <p className="o-m-0 o-text-base o-text-zinc-100">{etape.title}</p>
                          <p className="o-m-0 o-mt-2 o-text-sm o-leading-relaxed o-text-zinc-400">{etape.content}</p>
                        </div>
                      </li>
                    ))}
                  </ol>
          ...
          {/* ================= Le compte a rebours ======================== */}
          <section id="venir" className="o-scroll-mt-24 o-flex o-flex-col o-items-center o-justify-center o-border-t o-border-white-10 o-px-6 o-py-24 md:o-py-32" style={{ backgroundColor: accentDoux(700, 8) }}>
        ...
        <footer className="o-border-t o-border-white-10 o-px-6 o-py-16 md:o-px-12">
          <div className="o-mx-auto o-max-w-6xl">
            ...
            <div className="o-mt-12 o-grid o-gap-8 o-border-t o-border-white-10 o-pt-8 md:o-grid-cols-12">
              ...
              <nav aria-label="Rubriques" className="o-flex o-flex-wrap o-gap-x-8 o-gap-y-3 md:o-col-span-4">
                {(
                  [
                    ['#nuit', 'La nuit'],
