...
Le design n'a pas bougé. Ce qui a changé est le moteur, et ce qu'il fallait
pour l'atteindre.
...
| The framework's image component, the framework's link component, the framework's dynamic import | `<img>`, `<a>`, `React.lazy` |
| Utilitaires d'un générateur tiers | Utilitaires `o-`, plus les classes du projet |
| `@theme`, `@layer`, `@utility` | Des variables et des règles CSS ordinaires |

Mille classes sont passées par `scripts/lib/socle-classes.mjs`. Cette table n'a
pas été écrite à la main : elle est **dérivée** par
`scripts/derive-template-classes.mjs` du bloc de jetons du gabarit et de
l'échelle d'espacement de l'autre moteur. Soixante-dix de ces classes sont des
positions absolues au quart de rem près — de l'arithmétique, pas du jugement.

### Ce qui n'a pas suivi, et pourquoi

La route `app/api/contact`, `robots.ts`, `sitemap.ts` et la génération d'images
Open Graph demandaient un serveur. Les gabarits de la bibliothèque sont des
sites statiques. Pour un formulaire qui aboutit, voir le gabarit
`react-ts-server` de la CLI, qui pose un client et un serveur côte à côte.

`is-bot` lisait l'en-tête de la requête ; il lit désormais la même chaîne sur
le navigateur. Elle y est tout aussi déclarative, et tout aussi facile à
contrefaire — ce qui n'a jamais été le sujet : elle épargne à un robot le
chargement d'un morceau de page qu'il n'emploie pas.

### La rupture qui n'est pas la nôtre

Ce gabarit redéfinit ses points de rupture : `lg:` y vaut **1280 px**, pas
1024, et `tablet:` 540 px n'existe nulle part ailleurs. Toutes les classes qui
les portent sont donc des classes du gabarit, **y compris celles dont notre
système connaît l'utilitaire** — sinon elles se déclenchent au mauvais endroit.
Une grille de trois colonnes retombait à deux avant que ce soit fait.
