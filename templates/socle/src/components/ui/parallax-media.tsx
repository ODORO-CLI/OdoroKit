...
export interface ParallaxMediaProps {
  src: string;
  alt: string;
  /** Indication de tailles, transmise telle quelle a l image. */
  sizes: string;
...
const LAYER = {
  ...
  center: "o-absolute sn-inset-y-10 o-left-0 o-w-full",
  start: "o-absolute o-inset-y-0 o-left-0 o-w-full",
  // Bottom-anchored and 120% tall — keep in step with LIFT.
  "start-raised": "o-absolute sn-top-20 o-bottom-0 o-left-0 o-w-full",
} as const;
...
export const ParallaxLayer = ({
  ...
  <SpringTrigger
    ...
    className={`o-absolute o-inset-0 o-overflow-hidden ${className ?? ""}`}
  >
    {children}
  </SpringTrigger>
);
...
export const ParallaxMedia = ({
  ...
  <ParallaxLayer className={className} start={start} end={end} anchor={anchor}>
    {/* `fill` et `priority` etaient deux proprietes du composant image de
        l autre cadre. La premiere disait « remplis ton parent », ce qui
        s ecrit en CSS ; la seconde reglait la file d attente du chargement,
        ce qui s ecrit en attributs. */}
    <img
      src={src}
      alt={alt}
      sizes={sizes}
