...
**Langue** — toute la copie du parcours (titre, corps, boutons, `aria-label` de
fermeture, et les trois categories `necessary` / `analytics` / `marketing` avec
leur description) est en **francais**, alignee sur le reste du site. Les libelles
vivent dans les composants eux-memes, pas dans `src/data/` : `CookieBanner.tsx`
porte le texte de la banniere, `CookiePreferencesModal.tsx` la constante
`CATEGORIES` et les libelles de la modale. Toute traduction future se fait a ces
