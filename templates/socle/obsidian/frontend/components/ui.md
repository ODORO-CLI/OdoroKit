...
| `<BrandMark>` | `brand-mark.tsx` | Le sigle **ODORO** — coin carre en haut a gauche + arc de 270°, trace **geometriquement** dans une boite 100 × 100 (rayons 50 et 35, soit 15 d'epaisseur). Exporte `BRAND_MARK_PATH`, partage avec `icon.tsx`, `apple-icon.tsx` et `opengraph-image.tsx`, et repris a l'identique par le sigle 3D `public/assets/Mark/odoro-mark.glb` |
...
Le tracé a besoin de `fill-rule: evenodd` : sans lui les deux sous-tracés se
remplissent et le sigle devient une goutte pleine au lieu d'un anneau ouvert.
