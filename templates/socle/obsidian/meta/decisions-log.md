...
## ADR-0032 — La longueur des chaines de contenu est une contrainte de mise en page

- **Status:** Accepted
- **Date:** 2026-09-09

**Context.** Le site est reoriente : la page vendait UNE maison isolee, elle
presente desormais un programme neuf de promoteur immobilier, et la copie passe
en francais. Toute la copie visible vit dans deux fichiers, `src/data/mocks/home.ts`
et `src/lib/site.ts`, donc la reecriture ne touche aucun composant.

Mais la mise en page ne se comporte pas comme un document. Au-dessus de 1280 px
chaque section est `position: relative` a sa hauteur Figma et chaque enfant est
place en absolu a sa coordonnee, 1 px Figma valant 0,0625rem. Rien ne pousse
rien : une chaine plus longue ne repousse pas son voisin, elle deborde sa boite
ou chevauche l'element d'a cote. Le moteur de mots n'est pas utilise en dessous
de 1280 px, et au-dessus il dispose les mots en elements flex, donc un titre qui
tient a 1440 px peut gagner une ligne et decaler tout ce qui suit.

Le francais aggrave le probleme : a contenu egal il est couramment 15 a 20 %
plus long que l'anglais.

Deux chaines ont des contraintes plus dures encore. `hero.wordmark` est rendu a
801 px et le vault a deja mesure l'ecart entre la police de secours et la police
reelle sur ce mot precis (ADR du onzieme passage, 1909 px contre 1627 px) : le
nombre de lettres n'est pas libre, il reste a **5**. Et `numbers.stats[].value`
alimente un compteur qui incremente chiffre par chiffre, donc la valeur doit
rester une chaine numerique pure, l'unite allant dans `label`.

**Decision.** Toute chaine reecrite reste a **+/- 15 %** du nombre de caracteres
de l'originale, et les champs annonces comme maximum sont des maximums stricts
(`about.quote.text`, 28 caracteres, au-dela la carte deborde). Les champs
structurels — `href`, `id`, `cell`, `index`, `type`, `autoComplete`, `glyph`,
`src`, `width`, `height` — ne sont jamais touches par un travail editorial.

La reecriture est donc conduite comme une contrainte mesurable et non comme un
gout : chaque chaine est produite avec sa longueur, puis relue par une passe
d'edition qui recompte et raccourcit ce qui depasse.

**Consequences.** La copie francaise est plus dense que ce qu'un redacteur
ecrirait librement, et certaines formulations sont contraintes par le comptage
plutot que choisies. C'est le prix d'une mise en page au pixel, et le contrat de
la template le pose explicitement : `mutable = skin`, `preserve = motion,
composition`. Changer une composition de section est possible, mais c'est un
autre travail — les alternatives de meme role sont listees dans les
`recomposeOptions` de la template.

Corollaire pour les medias : les dimensions listees dans `MEDIAS-A-REMPLACER.md`
sont dans la meme categorie. Un rapport d'aspect different decale la mise en page
au-dessus de 1280 px, exactement comme une chaine trop longue.
