...
The scroll store (`src/hooks/smooth-scroll/use-scroll.ts`) is one of three
Zustand stores. The others are the cookie-consent store
(`components/common/Cookie/cookieStore.ts`) and the scene store
(`hooks/scene/use-scene-store.ts` — frames loaded, intro ready, dock hidden; see
[[hooks]]). Nothing per-frame lives in any of them: scroll-driven values stay on
springs ([[decisions-log]] ADR-0024). The scroll store's shape:
