...
see [[component-conventions]] hard rule #6. `HomeView` is the Keld Studio page.
It holds no state and no `"use client"`: it reads `homeContent` from
`src/data/mocks/home.ts` and hands each client leaf its slice.

## Current routes

| Route | File | View | Renders |
|-------|------|------|---------|
| `/` | `src/app/page.tsx` | `views/home.tsx` → `HomeView` | Keld Studio: preloader, pinned house scene, reviews (all inside `<main>`), then the reveal footer and floating dock as their own landmarks |

The sections live in `src/views/home/{preloader,scene,reviews,footer,dock}/`,
as feature leaves next to the view rather than in `components/`. `/` prerenders
static (`○`), and should stay that way. Anything that reads `headers()`, such
as `isBot()`, would make it dynamic, which is why the frame sequence's crawler
check runs client-side ([[decisions-log]] ADR-0025). The scene's scroll
architecture is [[decisions-log]] ADR-0024. The full story is in [[changelog]]
2026-09-14.
...
2. Create `src/views/<route>.tsx`, the actual page component, as a Server
   Component. Its client sections go in `src/views/<route>/<section>/`, the
   same shape as `views/home/`.
...
- `src/app/layout.tsx` — the **root layout**. Holds the provider tree
  ...
  loads the Instrument Sans font and `globals.css`, exports `metadata` + `viewport`, and
