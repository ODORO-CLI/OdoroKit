...
- **Scale down** (viewport ≤ 1440px): `vw`-based `html { font-size }` media
  queries in `globals.css` (`@layer base`), with bands at **1440 / 1024 / 640**.
  The 640 band is laid out on a 360 base. At each breakpoint's design base width
  the root font-size resolves to 16px; between breakpoints it tracks the viewport.
...
The starter's 1920 band was dropped for Keld Studio. The design is authored at
1440, so above 1440 the grid scales up instead of re-basing, like the other
1440-authored projects ([[changelog]] 2026-09-14).
...
Design-system primitives (`ArrowLink`, `GridLines`, `RevealLines`, the icons)
live in `src/components/ui/`. See [[components/ui]].
