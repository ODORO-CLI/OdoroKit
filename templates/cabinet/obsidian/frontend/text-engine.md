...
### 3. Positioning — wrap it, never position the engine itself

`TextEngine` writes **`position: relative` inline** on its container. An
`absolute` class on the `TextEngine` is therefore overridden, and every offset
you gave it (`top-1/2`, `right-10`, `bottom-10`) becomes a *relative nudge* —
words stack in flow and slide off the edge instead of landing in their slots.
It cost a QA round on the Keld Studio hero. Put the positioning on a wrapper:

```tsx
<span className="absolute right-10 top-1/2 -translate-y-1/2">
  <TextEngine tag="span" className="text-display leading-hero" …>in</TextEngine>
</span>
```

### 4. A clip needs a wrap layer to clip

`overflow` only acts on wrap layers that are **rendered**, and a layer is only
rendered when its `*In` target is non-empty. A letter that rises from under its
line (`letterOut={{ y: "105%" }}`) with no `wrapWordIn` has nothing to rise from
under. Pass a no-op layer — `CLIP_LAYER` in `src/lib/springs/presets.ts` — or
use `<RevealLines>` ([[components/ui]]), which does it for you.
...
`views/home/scene/` (hero blocks two and three, the ECHO® title).

> [!warning] For an entrance, use `toggle` — `interpolate` shows its first letters at progress 0
> Read from the 0.1.5 source: `interpolate` gives letter *i* the window
> `[i/n − coefficient, i/n]`, and progress is clamped to `[0, 1]`. Letter 0 is
> therefore **fully in at progress 0**, and the next few are partly in — measured
> on "Precision": `1.00 0.78 0.56 0.33 0.11 0 …` opacity before the scroll had
> reached the word at all. No trigger geometry fixes it. `toggle` tests
> `progress > i/n` (strict), so every letter is out at 0 and each springs in, on
> its own `letterConfig`, as the scroll passes it — and back out on the way up.
> `SCROLL_LETTER` in `src/lib/springs/presets.ts` is the config used. Keep
> `interpolate` for text that is meant to be partly visible from the start.
