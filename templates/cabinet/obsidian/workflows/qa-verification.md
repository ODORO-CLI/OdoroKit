...
| Group | Checks |
|-------|--------|
| Motion | `@keyframes`, foreign animation libs, `mode="manual"`, `leading-none`+`overflow`, bare `duration-fast` / `-normal` / `-slow` class (compiles to nothing), untokenised transitions |
| Tokens | hex in `className`/`style`, arbitrary px, literals in `@theme inline` or a Tier 2 token |
| Architecture | route importing outside `views/`, `"use client"` on `layout.tsx` / `page.tsx` / a top-level view (`src/views/*.tsx`), `any`, the framework's router, `middleware.ts`, `process.env` outside `env.ts` |
...
> [!note] Two false positives fixed — 2026-09-14
> - **Duration FAIL.** `\bduration-(fast|normal)\b` matched *inside*
>   `duration-[var(--duration-normal)]` — `\b` sits between the `-` and the `d` —
>   so every token-timed transition failed the check that prescribes that form.
>   It now requires a non-`-` before the match, `(^|[^-])duration-(…)`, and
>   covers `duration-slow` alongside the new 600 ms token.
> - **`"use client"` WARN.** It grepped `src/views/` recursively, so the feature
>   leaves that [[component-conventions]] place next to their view
>   (`src/views/home/scene/…`) all warned. It now checks `src/views/*.tsx` only —
>   the view itself must still be a Server Component.
>
> Context: [[changelog]] 2026-09-14.
