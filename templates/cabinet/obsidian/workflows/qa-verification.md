...
| Architecture | route importing outside `views/`, `"use client"` on `layout.tsx` / `page.tsx` / a top-level view (`src/views/*.tsx`), `any`, `next/router`, `middleware.ts`, `process.env` outside `env.ts` |
