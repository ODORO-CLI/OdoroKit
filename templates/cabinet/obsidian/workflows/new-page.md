...
> In the starter the home view (`src/views/home.tsx`, route `/`) ships **empty**;
> in an empty project, start there rather than scaffolding a new route. In this
> project it is already built (the Keld Studio page, sections in
> `src/views/home/`), so a new page means a new route.
