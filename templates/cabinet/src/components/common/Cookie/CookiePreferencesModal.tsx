...
const CATEGORIES: Category[] = [
  {
    ...
    body: "Indispensables au fonctionnement du site — sécurité, navigation entre les pages. Ils ne peuvent pas être désactivés.",
    required: true,
  },
  {
    key: "analytics",
    title: "Mesure d'audience",
    body: "Statistiques d'usage anonymisées, pour savoir quelles pages sont utiles. Aucun profil personnel n'est constitué.",
  },
  {
    key: "marketing",
    title: "Marketing",
    body: "Permet de mesurer nos campagnes et de vous proposer des contenus pertinents. Désactivable à tout moment.",
  },
];
...
export const CookiePreferencesModal = () => {
  ...
  return transitions((style, isOpen) =>
    isOpen ? (
      <animated.div
        className="o-fixed o-inset-0 cb-z-100 cb-font-sans"
        style={{ opacity: style.opacity }}
      >
        <div
          aria-hidden
          onMouseDown={closeModal}
          className="o-absolute o-inset-0 cb-bg-black-40 o-backdrop-blur-sm"
        />
        <animated.div
          role="dialog"
          aria-modal="true"
          ...
          className="o-absolute o-left-1/2 o-top-1/2 o-flex cb-max-h-calc-100dvh-1-5rem cb-w-calc-100vw-1-5rem cb-max-w-560px o-flex-col o-gap-5 o-overflow-hidden o-rounded-xl o-border-w-1 cb-border-foreground-10 cb-bg-background o-p-5 cb-text-foreground o-shadow-2xl sm:o-p-7"
        >
          <header className="o-flex o-items-start o-justify-between o-gap-3">
            ...
            <button
              ...
              className="o-flex o-h-8 o-w-8 o-shrink-0 o-items-center o-justify-center o-rounded-lg o-border-w-1 cb-border-foreground-10 cb-text-foreground cb-hover-bg-foreground-5 cb-focus-visible-outline-2 cb-focus-visible-outline-offset-2 cb-focus-visible-outline-foreground"
            >
          ...
          <p className="o-text-sm o-leading-relaxed cb-text-foreground-60">
            Choisissez les catégories de cookies que nous pouvons utiliser. Vous
            pourrez modifier ce choix à tout moment. Voir notre{" "}
            <a
              href="/privacy-policy"
              target="_blank"
              rel="noopener noreferrer"
              className="cb-text-foreground o-underline o-underline-offset-2"
            >
          ...
          <div className="o-flex o-min-h-0 o-flex-1 o-flex-col o-gap-3 o-overflow-y-auto o-py-1">
            {CATEGORIES.map((c) => {
              ...
              return (
                <div
                  key={c.key}
                  className="o-flex o-items-start o-justify-between o-gap-4 cb-rounded-10px o-border-w-1 cb-border-foreground-10 o-px-4 o-py-3.5"
                >
                  <div className="o-flex o-min-w-0 o-flex-col o-gap-1">
                    <h3 className="o-text-sm o-font-medium o-leading-snug">{c.title}</h3>
                    <p className="o-text-xs o-leading-relaxed cb-text-foreground-60">
                      {c.body}
                    </p>
                  </div>
                  <Toggle
...
const Toggle = ({ on, disabled, onChange, label }: ToggleProps) => {
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onChange}
      className={`o-relative o-h-6 o-w-11 o-shrink-0 o-rounded-full cb-focus-visible-outline-2 cb-focus-visible-outline-offset-2 cb-focus-visible-outline-foreground ${
        on ? "cb-bg-foreground" : "cb-bg-foreground-15"
      } ${disabled ? "o-cursor-not-allowed o-opacity-55" : "o-cursor-pointer"}`}
    >
      <animated.span
        style={{ transform: knob.x.to((v) => `translateX(${v}px)`) }}
        className="o-absolute cb-left-3px cb-top-3px o-block cb-h-18px cb-w-18px o-rounded-full cb-bg-background cb-shadow"
      />
    </button>
  );
};
