 ...
 *
 * **This used to be chrome, and it cannot be any more.** The plate was chromed
 * lettering on a near-black page, and chrome is only chrome when something
 * moves over it — a bright band on dark metal. The page is cream now, and the
 * plate is a flat olive watermark, so the same aperture reads the other way
 * round: the band DEEPENS the mark it carries instead of brightening it, and
 * what travels across the paper is a shadow rather than a highlight. The
 * mechanism is unchanged; only its sign is.
 *
 ...
 * and on a light ground that lift is already most of the effect: the watermark
 * goes from a soft sage tint to solid olive under the band.
 *
 * **The filter's sign is inverted from the dark original.** It was
 * `brightness(2.6)`, which brightened dark artwork so it read as light on
 * metal. Brightening olive on cream would wash it toward the paper and make
 * the band DISAPPEAR — the opposite of an effect. Deepening it instead keeps
 * the band reading as a shadow crossing the sheet, which is what a travelling
 * light does to ink rather than to chrome.
 */
const SWEEP_FILTER = "brightness(0.6) contrast(1.12)";
