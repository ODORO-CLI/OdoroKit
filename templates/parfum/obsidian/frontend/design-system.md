...
- **Tier 1:** the starter's neutral ramp is **gone** — every one of its
  primitives was orphaned by the re-dress and removed. What remains is the
  project's own palette (below) and two durations (`--raw-duration-fast/normal`).
- **Tier 2:** `--background`, `--foreground`, `--duration-fast`, `--duration-normal`.
  The `@media (prefers-color-scheme: dark)` override is **removed on purpose** —
  the page is one fixed light surface and the app shell follows it, so a 404 no
  longer renders as a dark card on the site's cream canvas. `color-scheme: light`
  is declared on `:root` so the UA scrollbar and form controls match.
- **Bindings:** `--color-background`, `--color-foreground`, `--font-sans`,
  plus `--leading-display` (1.1 — the clip floor for [[text-engine]]) and
  `--ease-entrance`.

### This project's palette — Olive Terracotta (ADR-0049)

The brand palette is the GetLayers `olive-terracotta` ramp, a LIGHT warm set,
held in Tier 1 as `--raw-color-cream` (#efe0cd), `--raw-color-cream-deep`
(#e3d3bd), `--raw-color-ink` (#0b0b0e), `--raw-color-olive` (#607556),
`--raw-color-terracotta` (#b96a4c) and `--raw-color-oxblood` (#7b2524), with the
ink at two further alphas (`-ink-58`, `-ink-40`), the olive at one (`-olive-50`),
and nine `--raw-color-glow-*` stops for the pointer torch. The torch used to be
six `rgb(255 255 255 / …)` literals written out twice; tokenising it is what
keeps the panel copy of the lattice in phase with the page behind it.

### This project's additions

The hero frame added a `--hero-*` role set — `surface` (the lattice BAR and
the html canvas), `surface-lattice` (the CELL, always the lighter of the two —
the grid *is* that luminance step), `content` / `content-muted` /
`content-faint` (one ink at three alphas), `rule` (every hairline, in olive —
it diverged from `faint` with the re-dress), `accent` (terracotta, rationed to
CTA rules, the pointer light and the WebGL key), `display` (oxblood, for the
four section mastheads only), a 12/18/20px type ramp, the 0.9 and 1.1 leadings,
and the −0.02em tracking. These roles are **deliberately not overridden** by any
colour-scheme query: the frame is a fixed **light** canvas by design (it was a
fixed dark one until ADR-0049).
...
|------|--------|----------|---------|---------|
| **Onest** | the framework's font loader | `--font-onest` | `--font-sans` | Site default |
| **IBM 3270** | the framework's local font loader (`src/app/fonts/3270-Regular.otf`) | `--font-3270` | `--font-mono` | The Get Layers hero frame |
