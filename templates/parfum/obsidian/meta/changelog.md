...
## 2026-09-09 (repo) — Le dossier devient autonome et prêt à pousser

- `assets-source/` : les rendus à étiquette vierge (hero, quatre flacons,
  pyramide) en WebP sans perte, avec un README qui dit comment chaque asset servi
  est produit à partir d'eux.
- `.claude/scripts/assets/label-packshot.py` : le script d'étiquetage, chemin de
  police relatif au dépôt.
- `README.md` reçoit un en-tête ODORO (démarrage, carte du projet, vérifications,
  réserves avant lancement) au-dessus de la doc du starter ; `HOW_TO_USE.md`
  remplace celui d'Artefakt.
- Retirés : `hero-logo.png` et `hero-wordmark-dark.png`, référencés nulle part.
- `.claude/scripts/video/README.md` note que la vidéo source du reel a disparu
  avec la conversion parfum.
- Premier commit du dépôt.

## 2026-09-08 (3D, 2) — Les étiquettes sont écrites par la page, plus par le générateur

Signalé : le texte des étiquettes était illisible sur le flacon du hero et sur les
quatre flacons de la collection. Cause : la génération d'images rend les petits
textes en glyphes inventés, et ce texte était cuit dans la texture du modèle et
dans les packshots. Remède : ne plus laisser l'IA écrire.

**Changed**
- Tous les rendus sont régénérés avec une **étiquette vierge**. Le modèle 3D
  (`hero-flacon.glb`, 659 Ko après Draco + WebP) porte un papier crème nu.
- `hero-scene.tsx` : un **plan d'étiquette** enfant du modèle, texturé par un
  canvas dessiné dans la 3270 lue via `--font-3270`, placé en fractions de la
  boîte du modèle (`LABEL`) pour survivre à une regénération. Papier légèrement
  émissif pour rester crème sous la clé terre cuite. `metalness` forcé à 0 et
  carte de métallicité ignorée : la passe PBR du générateur transformait le
  verre en feuille d'or froissée.
- Les quatre packshots reçoivent leur vrai texte par un script PIL qui détecte le
  rectangle de papier (forme contrainte : plus large que haut, < 36 % de la
  hauteur, rempli) et repli sur le rectangle des trois autres quand le liquide
  pâle se confond avec le papier (l'iris). Le papier est forcé opaque, le
  détourage ayant laissé le flacon pâle semi-transparent.
- `--raw-filter-reel-tint` passe à `none` : la pyramide olfactive est générée
  dans la palette et la rotation de teinte des anciennes images la rendait
  turquoise.

## 2026-09-08 (3D) — Le flacon est cadré, éclairé et documenté comme un flacon

Suite d'ADR-0050 : la scène chargeait bien le flacon, mais avec les constantes
de la veste — presque carrée, cadrée pour déborder du bas. Un rendu réel a montré
le capuchon coupé dans le hero, l'étiquette hors cadre dans les détails et le
flacon poussé hors champ à gauche dans la FAQ.

**Changed** (`hero-scene.tsx`, `faq-stage.tsx`)
- Hero : `SUBJECT_HEIGHT` 570 → 470, `SUBJECT_RISE` 36 → 58 — l'objet entier,
  étiquette au centre optique, plaque de fond lisible de part et d'autre.
- Détails : `DETAILS_HEIGHT` 795 → 640, `DETAILS_DROP` 140 → 30,
  `DETAILS_SHIFT_X` -92 → -60 — le flacon reste entier au lieu d'être coupé.
- FAQ : `STILL` 765/-285/-140 → 560/-300/-40, approche 28°/110 → 18°/90.
- `SCROLL_TURN` 40° → 24° : la vue trois-quarts servait la forme d'une veste ;
  sur un flacon elle écrasait l'étiquette, seul texte de l'objet.
- **Toute la documentation des composants 3D est réécrite** pour l'objet réel :
  le moteur de « jiggle » (chaînes d'os, gains `sleeveLag` / `travelLag` /
  `dragLag`) est explicitement marqué dormant — le flacon est un maillage sans
  squelette, `collectJiggle` ne trouve rien — et conservé comme moteur générique
  avec son historique de réglage. Les notes d'axe de cadrage disent désormais
  que la hauteur lie sur toutes les boîtes. `LOOK.colour` est documenté comme
  albédo de repli, non comme ce que le flacon montre.

## 2026-09-08 (perfume) — Retour au parfum, avec un vrai flacon 3D et une copie française

Voir [[decisions-log]] ADR-0050. La direction artistique reste le papier olive et
terre cuite, lu comme une officine d'apothicaire.

**Changed**
- `public/assets/hero/hero-flacon.glb` remplace `hero-jacket.glb` : flacon généré
  (packshot → Meshy image-to-3D), compressé Draco + WebP, 490 Ko. Les deux
  `subject.src` pointent dessus.
- `hero-scene.tsx` : l'upgrade de matériau garde les textures du modèle (albédo
  blanc, maps couleur/rugosité/normales), coupe l'irisation, monte la sonde.
- `technology-reel.tsx` : `count: 1` dans les deux paliers ; le reel est une image
  fixe de la pyramide olfactive. Les 102 anciennes images sont supprimées,
  `manifest.json` suit.
- `public/assets/collections/flacon-{oud,vetiver,ambre,iris}.png` : quatre
  déclinaisons du flacon, détourées, 1080×1080. Les quatre doudounes sont retirées.
- `src/data/mocks/home.ts`, `site.ts`, `manifest.json` : tout en français, écrit
  aux longueurs de l'artboard. `layout.tsx` passe en `lang="fr"`.

## 2026-09-08 (re-dress) — Olive Terracotta: the page goes from near-black to cream paper

The user asked for a different visual while keeping a site that sells jackets.
Going back to jackets made every asset correct again — the model, the reel and
the four photographs are all a jacket — so the change is the palette and what
follows from it, not the composition. Direction chosen by the user from four
real GetLayers palettes: `olive-terracotta`, a LIGHT warm ramp. See
[[decisions-log]] ADR-0049.

**Why the type did not change.** Every text element on the page, the 70px
mastheads included, decodes through `ScrambleText`, which substitutes glyphs in
place and relies on a monospace so that nothing reflows mid-decode. Seven
headlines are also `text-box: trim-both` against hard `lg:top-*` offsets, so a
new face would move all of them. IBM 3270 stays; the change is colour.

**Changed — tokens** (`src/app/globals.css`)
- Tier 1: the starter's neutral ramp and the frame's four hero primitives were
  all orphaned and removed. In their place: cream / cream-deep / ink / olive /
  terracotta / oxblood, the ink at two more alphas, the olive at one, a
  `--raw-color-shell` for the garment, a `--raw-filter-reel-tint`, and nine
  `--raw-color-glow-*` stops.
- Tier 2: `--hero-surface` (bar + canvas) and `--hero-surface-lattice` (cell)
  keep their roles with the cell still the lighter; `--hero-rule` diverges from
  `faint` and becomes olive; new `--hero-accent`, `--hero-display`,
  `--hero-shell`, `--hero-reel-tint`. `--background` / `--foreground` follow the
  page onto cream so a 404 stops being a white card on the site's canvas.
- **The pointer torch is tokenised.** It was six `rgb(255 255 255 / …)` stops
  written out twice with nothing keeping the two copies in step — the mapping
  sweep's first "what breaks first". It is terracotta now, at alphas scaled 2.6x,
  because additive white is invisible on a 90%-luminance ground.
- The `prefers-color-scheme: dark` override is removed on purpose;
  `color-scheme: light` is declared; `::selection` is styled (it was the one
  UA-blue accident the design did not own).

**Changed — WebGL** (`hero-scene.tsx`)
- `LOOK.key.colour` azure → terracotta, intensity 4.5 → 5.2 (terracotta carries
  its energy across three channels where azure clipped one). The shell goes from
  blue-black to a warm near-black lifted to RGB 26 — on cream an albedo near
  zero reads as a cut-out, not an object. The rim inverts its job: a dark olive
  edge separates the form where a bright one now erases it. Iridescence
  thickness 360 → 260nm so the film's interference sits in the key's family
  instead of cancelling it. Every held value carries a comment saying why.
...
- **The technology reel's frames are re-baked.** They carried a soft band of
  near-black pixels at ~30% alpha — invisible on black, a grey smear on cream.
  A steepening alpha curve is now applied to all 102 frames and the poster
  (lossless), which also makes the poster-to-canvas hand-off match for real.
  The poster's runtime SVG key filter, which the mapping critic found was
  driving already-keyed pixels below full opacity, is removed. ADR-0046 is
  amended accordingly.
- The reel is warmed toward the palette through `--hero-reel-tint` (its frames
  are magenta-violet at export and cannot be recoloured in code).
- Brand rasters regenerated for the light ground: the logotype is ink, the
  backdrop plate is a flat olive watermark (chrome is a dark-page effect; the
  wordmark's sweep filter inverts its sign to match), the share image is on the
  cream lattice. The icon set — favicon, apple, android — was still the previous
  project's blackletter mark; it is one ODORO monogram now, plus the three
  `ms-icon` tiles `browserconfig.xml` always pointed at and the two
  `apple-touch-icon` names Safari probes. `manifest.json` and `themeColor` carry
  the page ground.

**Changed — copy** (`src/data/mocks/home.ts`, `site.ts`)
- Back to outerwear, in a warmer voice. Written to the frame's line lengths
  again; the adversarial verify pass caught four that were not (two layer
  bodies that would have grown their cards past the measured `cardSideY`, an
  FAQ question that fell to three lines, a masthead whose tied pair overhung
  its column) and they are corrected here.

**Environment note.** The bake step for the reel frames is PIL in a one-off
script; `.claude/scripts/video/frames.mjs` remains the source of the frames
themselves and does not yet apply the alpha curve — re-exporting from the mp4
would need it re-applied.

## 2026-09-08 (brand) — Re-skinned from ARTEFAKT to ODORO, a fragrance house

The page arrived as the GetLayers **Artefakt** template — a technical outerwear
label — and was re-skinned for **ODORO**, an extrait-de-parfum brand. The Style
did not change: Artefakt's own tokens are the committed Style, so nothing was
re-tinted. What changed is copy, the brand's three raster assets, and the logo's
box.

**Why the template carries a perfume at all.** Its spine is a physical object
rendered live and walked down the page, plus a pinned stage that scrubs a card
down five separated layers on a leader line. A fragrance already has five layers.
The olfactory pyramid maps onto the material stack one-for-one, which is why the
composition needed no restructuring — see `homeTechnology`.

**Changed**
- `src/data/mocks/home.ts` — every string rewritten for ODORO. **Copy was
  written to the artboard's own lengths**, within a character or two per line, so
  the frame's hand-set boxes still break where they were designed to. Three
  constraints were load-bearing and are recorded in the file: the nav label stays
  `COLLECTIONS` (its `li` reserves a 119-unit caret box measured against that
  word), the technology heading's first line is held to nine characters
  (`CHEMISTRY`) so it still sets on one line in the 367-unit column, and FAQ row
  02 keeps `questionWidth: 195` with a question long enough to still need two
  lines. Every `cardTop` / `cardSideY` / `anchorFx` / `anchorFy` is **untouched** —
  they are measured against the reel's artwork, not derived.
- The artboard's own `WORLOWIDE SHIPPING` typo is **corrected**. It was preserved
  before because the copy was a transcription; it is not one any more.
- `src/lib/site.ts` — `name`, `description` and `author` are ODORO's.
- `public/assets/ui/logo-mark.png`, `public/assets/hero/hero-wordmark.png`
  (+ `-dark`), `public/assets/hero/hero-logo.png`, `public/open-graph.png` —
  regenerated. **The ODORO logotype is set in the page's own 3270 mono**, which
  is what the Style asks for ("one monospaced face doing every job"); ARTEFAKT's
  was a separate display face we do not have and would not have wanted twice.
  The chrome plate keeps the original's 5028x2684 canvas and its 4634-unit ink
  span, so `INK_SCALE` in `hero-wordmark.tsx` stays valid, and its strokes peak
  at RGB 250 against the original's 251 — `BACKDROP_OPACITY` needs no change.
- `obsidian/frontend/seo-metadata.md` — its `#todo` claimed `open-graph.png` was
  900x600 and that the metadata agreed. Both halves were stale: the file is
  1200x630 and `generate-page-metadata.ts` has always declared 1200x630. Measured
  and corrected while regenerating the plate.
- `hero-header.tsx` / `footer.tsx` — the logo box keeps the frame's **widths**
  (75 / 110 and 99 / 130) and lets height follow the new 5.295 artwork ratio.
  Keeping the width means no horizontal coordinate on the artboard moved; the
  shorter mark also lands on the nav's own optical line at `top-6`.

**Still placeholder — flagged in the source, not hidden**
- `hero-jacket.glb` is the template's garment, not a flacon. Its a11y label says
  so, deliberately: a screen reader must not be told there is a bottle on screen.
- The 61-frame technology reel is the garment's materials exploding, and the four
  collections photographs are four colourways of that garment.
- The four prices are invented and carry only the card's hover state.

**Environment note.** Installed with `npm`, not `yarn` — neither yarn nor corepack
is present on this machine. `three` and `@types/three` are the only dependencies
the template adds over the bare starter.

**Watch the lockfiles.** `npm install` silently **rewrote `yarn.lock`** into npm's
own dialect: `registry.npmjs.org` URLs with the integrity hash dropped from the
fragment, 120 KB against the original's 161 KB. It resolved the same tree, but a
lockfile without integrity hashes is a weaker guarantee than the one the template
shipped, so the original was restored from the template and is what is committed.
`package-lock.json` sits beside it and records what is actually in `node_modules`
right now. If you install with npm again, expect `yarn.lock` to be rewritten
again — check it before committing.
