...
## ADR-0050 — ODORO est une maison de parfum ; le sujet 3D est un flacon généré, la copie est française

**Status:** Accepted · 2026-09-08 · amends ADR-0049 (palette held), ADR-0046 (the reel)

**Context.** The brief returned to the perfume idea that had been abandoned for
want of a bottle: sell perfume, keep the 3D stack, a real perfume art direction,
and every word in French with proper copywriting. The blocker last time was that
the only product model on disk was a jacket.

**Decision.**
- **The art direction stays on the Olive Terracotta paper, read as apothecary.**
  Cream paper, olive rules, a monospace label face, an amber-glass flacon with an
  olive-black bakelite cap: this is the niche-perfumery register (Le Labo, Aesop)
  and it needed no new palette. The Style's disciplines — one ink at four alphas,
  colour as light, no fills — are unchanged.
- **The flacon is a generated asset, not a hand-authored scene.** A packshot was
  generated (Higgsfield / nano_banana), then lifted to a textured PBR GLB
  (Meshy image-to-3D), then compressed with gltf-transform: Draco geometry and
  WebP textures at 1024, 9.2 MB → 490 KB. `GLTFLoader` reads both natively and
  the Draco decoder already ships in `public/draco/`. The four collection
  flacons and the olfactory-pyramid still were generated from the same packshot
  as reference so the set coheres, then background-removed to transparency.
- **The scene's material upgrade inverts its logic.** The jacket path replaced
  every material with one dark lacquer (`LOOK.colour`); the flacon arrives with
  its own base-colour, metal/roughness and normal maps, and the glass, cap and
  label all live IN those maps. The albedo is now white, the maps carry the
  colour, the clearcoat stays for the wet glass, the probe is raised, and the
  thin-film iridescence is switched off — on amber glass it read as a soap
  bubble. Lights, tone mapping and framing are untouched; framing is
  height-based so a taller, narrower object fits without new constants. The
  jiggle rig finds no bones and stands down by design.
- **Type on the object is the page's, never the generator's.** Generated
  images render small text as invented glyphs, so every asset is generated with
  a blank label and the real text is applied afterwards: a canvas-textured
  plane in 3270 on the 3D model, PIL-drawn 3270 on the four packshots. One
  typeface from the nav to the label on the bottle.
- **Framing is re-measured for a tall, narrow object.** Hero 470 units (was
  570), details 640 whole rather than 795 cropped, FAQ 560 whole and cropped
  by one edge only, scroll turn 24° (was 40°) so the label stays legible. The
  jiggle engine stays in the file, documented as dormant: the flacon has no
  bones, and the engine is the reusable part.
- **The reel is one still, `count: 1` in both tiers.** The 61-frame explosion
  was the jacket's materials; a perfume's five layers are the pyramid, composed
  as one image in five horizontal strata to receive the existing
  `anchorFx`/`anchorFy`. The scrub resolves to frame 0 at every playhead; the
  card alone carries the motion. This amends ADR-0046 a second time.
- **French is written to the artboard's lengths, like the English was.** The
  three load-bearing constraints (a `COLLECTIONS` nav label, a ≤10-character
  technology heading, a two-line FAQ 02) all survive translation; `COLLECTIONS`
  is the same word. `Œ` is written `OE` because the 3270 has no ligature.
  `<html lang="fr">`.

**Constrains.** The generated assets carry no licence text beyond the
generator's terms; check them before a commercial launch. Re-generating the
flacon changes the texture's label, so the a11y labels and the OG plate must be
re-read. The four prices remain invented.

## ADR-0049 — The page is cream paper: Olive Terracotta, with colour as the only variable

**Status:** Accepted · 2026-09-08 · amends ADR-0046 (the reel's key) and ADR-0048 (the logotype's colour) · **amended by ADR-0050** (2026-09-08): palette held, subject becomes a perfume flacon, copy in French

**Context.** The brief was a different visual for a site that still sells
jackets. Four real GetLayers palettes were put to the user; they chose
`olive-terracotta`, a light warm ramp — cream ground, one near-black ink, olive
rules, terracotta accent, oxblood mastheads.

**Decision.** The page flips tone, dark → light, through the token tiers alone.
Composition, motion and typeface are held. Specifically:

- **Type is held because the decode effect requires it.** Every text element,
  mastheads included, goes through `ScrambleText`, which swaps glyphs in place
  and depends on a fixed-pitch face so nothing reflows while it runs. Seven
  headlines are also `text-box`-trimmed against hard offsets. A proportional
  display face would have needed either a rewrite of the effect or a retreat
  from it on the headlines; neither is a colour change.
- **The grid is a luminance step, not a colour.** `surface` stays the bar and
  the canvas, `surface-lattice` stays the cell, and the cell stays the lighter
  of the two — cream over deep cream, as it was #0d0d0d over black.
- **The ink is one ink at four alphas, with the alphas re-measured.** Ink on a
  90%-luminance ground needs more paint than white on black for the same
  perceived step, so 40/25 became 58/40, and the rule diverged to olive at 50%.
- **The one hot hue is the light, still.** Terracotta is the WebGL key, the
  pointer torch and the CTA rule — never a fill. The torch is tokenised, and it
  is terracotta rather than white because additive white on cream is invisible.
- **The shell has its own primitive.** The garment's albedo is a material
  value, not a UI colour, and the page ink was the wrong answer for it (too
  cool, too deep on a bright-adapted eye). Rather than let a `.tsx` invent a
  hex, `--raw-color-shell` / `--hero-shell` hold it and the scene mirrors it.
- **Baked assets are re-baked, not filtered.** The reel's frames carried a
  near-black mid-alpha band that only a dark ground could hide. The fix is an
  alpha curve applied to the files, so the poster and the canvas match for
  real and no runtime filter is needed for the key. This supersedes the half of
  ADR-0046 that said "the filter survives only for the poster" — the poster is
  keyed too, and the filter is gone. The colour warm-up that *is* a runtime
  filter is held in a token, `--hero-reel-tint`, so it moves with the ramp.

**Constrains.** Any future palette lands in Tier 1 and nowhere else; the two
literals in `hero-scene.tsx` (`key.colour`, `colour`) and the SVG fills under
`public/assets` are the three places outside CSS that mirror a primitive and
must be moved by hand when it moves. Re-exporting the reel from the mp4 must
re-apply the alpha curve. `manifest.json`, `browserconfig.xml` and
`siteConfig.themeColor` carry the page ground as literals and must move together.

## ADR-0048 — The ODORO logotype is the page's own mono, and the logo box keeps its width

**Status:** Accepted · 2026-09-08 · project decision, on top of the inherited set

**Context.** The page was re-skinned from ARTEFAKT to ODORO. ARTEFAKT's mark was
a raster in a heavy display face that ships with neither the starter nor the
template — only the 300x120 PNG of it does. Something had to set the new name.

**Decision.** The ODORO logotype is **set in 3270**, the single monospaced face
the page already uses for everything from the loading readout to the 70px
headlines, with 0.08em tracking, and rendered to `logo-mark.png` at its own ink
ratio (5.295) rather than into the old asset's 2.5 box.

**Why not source a condensed display face.** The Style's own rule is one
monospaced face doing every job, and "a second ink breaks it" is the same
argument one step over. Buying a display face to set five letters would put a
second type system on a page whose identity is that it has one. The 3270 mark
also reads as a *terminal* wordmark, which is the register the rest of the page
is already in.

**Why the box keeps its WIDTH, not its height.** Five mono letters are far wider
per unit of height than eight condensed ones — 5.295 against 2.5. Holding the
frame's 30-unit height would have made the mark 159 wide against the artboard's
75, and every horizontal coordinate on this page is anchored to an edge or the
centre precisely so that widths are the thing that must not move. Holding the
**width** instead (75 at `lg`, 110 below; 99 / 130 in the footer) moves no
coordinate at all, and the resulting 14-unit height lands the mark on the nav's
own optical line at `top-6` rather than above it.

**Constrains.** `LOGO_BOX` in `hero-header.tsx` and the footer's logo link are a
matched pair: change the artwork's ratio and both heights have to be recomputed
from the kept widths. The chrome backdrop plate is a separate constraint —
`hero-wordmark.png` must keep the 5028x2684 canvas and its 4634-unit ink span or
`INK_SCALE` in `hero-wordmark.tsx` is wrong, and its strokes must stay near RGB
251 or `BACKDROP_OPACITY` in `hero-stage.tsx` is wrong. Both are documented
where they are set.
...
**Status:** Accepted · 2026-08-28 · supersedes [[decisions-log]] ADR-0042, · **amended by ADR-0049** (2026-09-08): the poster is keyed at export too, its runtime SVG key filter is removed, and all 103 files are re-baked through an alpha curve for the light ground · **amended again by ADR-0050** (2026-09-08): the reel is one still of the olfactory pyramid, `count: 1`
