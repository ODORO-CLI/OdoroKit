...
> **This page is now ODORO, a fragrance house.** The node IDs below still describe
> the COMPOSITION — every box, coordinate and break is the artboard's and is
> unchanged. The COPY is no longer the frame's: it was rewritten for ODORO to the
> same line lengths, so the breaks these nodes were designed around still hold.
> Verify layout against these nodes; do not verify wording against them.
> See [[changelog]] 2026-09-08 and ADR-0048.
>
> **And the COLOUR is no longer the frame's either.** The artboard is a
> near-black lattice lit by one azure key. The page is now cream paper ruled in
> olive with a terracotta key (ADR-0049). Geometry, boxes and breaks are still
> the frame's; every colour in the table below should be read through the
> `--hero-*` roles, not the hexes the frame was sampled at.
>
> **And the SUBJECT is a perfume flacon, in French** (ADR-0050): where the table
> says jacket, garment or sleeve, read flacon; the model has no bones, so the
> sleeve-spring rows describe a mechanism that now idles by design.
