...
export const CookiePreferencesModal = () => {
  ...
  return transitions((style, isOpen) =>
    isOpen ? (
      <animated.div
        className="o-fixed o-inset-0 hl-z-100 o-font-sans"
        style={{ opacity: style.opacity }}
      >
        <div
          aria-hidden
          onMouseDown={closeModal}
          className="o-absolute o-inset-0 hl-bg-black-40 o-backdrop-blur-sm"
        />
        <animated.div
          role="dialog"
          aria-modal="true"
          ...
          className="o-absolute o-left-1/2 o-top-1/2 o-flex hl-max-h-calc-100dvh-1-5rem hl-w-calc-100vw-1-5rem hl-max-w-560px o-flex-col o-gap-5 o-overflow-hidden o-rounded-xl o-border-w-1 hl-border-foreground-10 hl-bg-background o-p-5 hl-text-foreground o-shadow-2xl sm:o-p-7"
        >
          <header className="o-flex o-items-start o-justify-between o-gap-3">
            ...
            <button
              ...
              className="o-flex o-h-8 o-w-8 o-shrink-0 o-items-center o-justify-center o-rounded-lg o-border-w-1 hl-border-foreground-10 hl-text-foreground hl-hover-bg-foreground-5 hl-focus-visible-outline-2 hl-focus-visible-outline-offset-2 hl-focus-visible-outline-foreground"
            >
          ...
          <p className="o-text-sm o-leading-relaxed hl-text-foreground-60">
            ...
            <a
              ...
              className="hl-text-foreground o-underline o-underline-offset-2"
            >
              privacy policy
            </a>
            .
          ...
          <div className="o-flex o-min-h-0 o-flex-1 o-flex-col o-gap-3 o-overflow-y-auto o-py-1">
            {CATEGORIES.map((c) => {
              ...
              return (
                <div
                  key={c.key}
                  className="o-flex o-items-start o-justify-between o-gap-4 hl-rounded-10px o-border-w-1 hl-border-foreground-10 o-px-4 o-py-3.5"
                >
                  <div className="o-flex o-min-w-0 o-flex-col o-gap-1">
                    <h3 className="o-text-sm o-font-medium o-leading-snug">{c.title}</h3>
                    <p className="o-text-xs o-leading-relaxed hl-text-foreground-60">
                      {c.body}
                    </p>
                  </div>
                  <Toggle
...
const Toggle = ({ on, disabled, onChange, label }: ToggleProps) => {
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onChange}
      className={`o-relative o-h-6 o-w-11 o-shrink-0 o-rounded-full hl-focus-visible-outline-2 hl-focus-visible-outline-offset-2 hl-focus-visible-outline-foreground ${
        on ? "hl-bg-foreground" : "hl-bg-foreground-15"
      } ${disabled ? "o-cursor-not-allowed o-opacity-55" : "o-cursor-pointer"}`}
    >
      <animated.span
        style={{ transform: knob.x.to((v) => `translateX(${v}px)`) }}
        className="o-absolute hl-left-3px hl-top-3px o-block hl-h-18px hl-w-18px o-rounded-full hl-bg-background hl-shadow"
      />
    </button>
  );
};
