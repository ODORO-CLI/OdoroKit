...
Two composed wrappers in `src/components/animation/` package the redesign's
"soft entrance" so every section shares one motion language (letters/words fly up
from below with a blur→sharp and opacity ramp):

| Component | Granularity | Use for |
|-----------|-------------|---------|
| `<AnimatedHeading>` | per **letter** | the mirror-gradient mastheads (hero/services/timeline). Splits each line into per-letter `useSpring` spans; because `background-clip:text` cannot survive the split, every letter takes a **solid colour sampled from the gradient** (white at the opaque end → pale haze at the transparent end). The cascade starts at each line's `opaque` end and travels inward, so the two mirrored lines animate from their bright corners. |
| `<AnimatedText>` | per **word** | body copy (subtitles, footnotes). Thin wrapper over [[text-engine]] with `wordIn/wordOut` blur + fly-up. |

### Desktop hovers & the mobile reflow

- **Hovers** on every interactive control (nav pills + links, both CTAs, the
  contact-form submit) use `<Hover>` — a spring `from`→`to` that reverses on
  mouse-leave and is **auto-disabled on touch** (`springsConfig.disableOnMobile`).
  Pills/CTAs lift with `scale`; nav links brighten (`opacity 0.68 → 1`). The
  contact form is shared (`src/views/home/sections/contact-form.tsx`, `layout`
  `"row"` desktop / `"stack"` mobile) so every CTA lifts identically.
- **Mobile reflow.** The four sections render their scaled 1440×800 desktop
  composition only `≥ hero-md`; below it (`useIsMobile`, `src/hooks/use-is-mobile.ts`,
  threshold 856px) each swaps to a native vertical layout — centred stacks for
  hero/services/logo, a burger sheet for the nav, and a scroll-driven **vertical**
  rail for the timeline (the same `useSlideProgress` translated on Y instead of X,
  travel measured from the DOM). `useIsMobile` is SSR-safe (width 0 → desktop), so
  server and client markup match; the swap lands on mount, hidden behind the
  preloader.
