...
## ADR-0027 — One lockfile: `package-lock.json`. `yarn.lock` is deleted

**Status:** Accepted · 2026-09-15 · settles the open question in ADR-0026

**Context.** ADR-0026 left both lockfiles in the tree and said to pick one
before the first production deploy. That deploy is now the next step, so the
question is due.

The risk is not theoretical. **Vercel selects the package manager by which
lockfile it finds**, and a `yarn.lock` wins. The tree that was installed,
type-checked, linted, built, screenshotted and judged this session came from
`package-lock.json` via `npm ci`. A Yarn install resolves its own tree — same
declared ranges, different resolution — and nothing that was verified here
would have been verified there. Two lockfiles is two possible sites.

**Decision.** `yarn.lock` is deleted. `package-lock.json` is the only lockfile;
`npm ci` is the install; the README says so. Adding a `yarn.lock` back is a
decision that needs its own ADR, and it means re-running the verification.

**Consequences.** Contributors use npm. The `engines` floor (Node ≥ 20.19) is
what CI must satisfy. If Yarn is ever wanted, delete `package-lock.json` in the
same commit — never both.

---

## ADR-0026 — npm is the package manager in this checkout

**Status:** Accepted · 2026-09-15 · project decision

**Context.** The starter is a Yarn project (`yarn.lock`, README). The machine
this project was built on has a Volta shim for `yarn` but no Yarn installed,
and `corepack` is absent; `npm` 11 is present.

**Decision.** Dependencies are installed with `npm install`; the resulting
`package-lock.json` is committed. Scripts are unchanged, so
`npm run dev|build|lint` is the invocation.

> [!note] Settled by [[decisions-log#ADR-0027|ADR-0027]]
> This ADR originally left `yarn.lock` in place beside it, to be resolved
> before the first deploy. It has since been deleted — there is one lockfile.

**Consequences.** Docs that say `yarn <script>` read as `npm run <script>`
here. The `engines` field (Node ≥ 20.19) is satisfied by Node 25.

---

## ADR-0025 — Page chrome and the film are painted from the shared ticker, not rendered from state

**Status:** Accepted · 2026-09-15 · applies ADR-0002 to scroll-driven chrome

**Context.** The header must be up only on the first screen, the floating pill
only between the first screen and the closing one, and the film must zoom,
drift and gather a scrim with the scroll. All three are continuous functions
of the scroll position, evaluated every frame.

**Decision.** `<SiteChrome>` subscribes once to `subscribeToTicker` (the
supported extension point, [[animation-system]]), reads `window.scrollY` and
the closing section's rect, and toggles two bare `SpringValue`s whose
`onChange` writes opacity / transform / `clip-path` straight to the DOM.
`<HeroFilm>` composes a bare entrance `SpringValue` with the `<ProgressTrigger>`
progress in one `paint()` that writes the video's transform and the scrim's
opacity. No React state is touched by a scroll frame. The same pattern holds
for the lookbook ring (a rAF engine writing one `--rot`) and the swarm (a
render loop on the ticker).

**Consequences.** A scroll never re-renders the tree. Anything that reads
these values must read the DOM or the spring, not a store. The vendored
`<Spring>` / `<Inview>` components are still what starts *discrete* entrances
(`--stage-reveal` on a layer, a block rising into view); the ticker path is
only for values that change every frame.

---

## ADR-0024 — The page is GetLayers compositions + one Style + assets; type resolves from one custom property

**Status:** Accepted · 2026-09-15 · project decision

**Context.** The brief asked for a video-backed, parallax, text-revealing,
depth-forward clothing-brand page. GetLayers supplies layout skeletons
(compositions), a Style (tokens + type + treatment), library Sections and
Scenes with a `mutable` / `preserve` contract, and a reveal choreography
harvested from shipped templates.

**Decision.** Every section is built to a named composition (`loopstack-hero`,
`negantropy-entropy/choice`, `artist-statement`, `vexon-showcase`,
`carousel-spotlight`, `ai-studio-cta`, `artist-footer`) under the one committed
Style `ai-creator-style`; the choices are recorded in `getlayers.json` and
re-read before any section is touched. Library assets keep their preserved
layers: the lookbook ring's engine is ported verbatim and only re-skinned; the
`onyx-cubes` scene is tinted through its CONFIG (colours read from the page's
tokens at mount), never through its shaders.

**Type over pinned layers** (the curtain, the film's first screen, the closing
screen) resolves through one custom property, `--stage-reveal`, written by a
`<Spring>` / `<Inview>` and divided up in CSS by `<StageReveal>` (per letter or
word, scrambled but deterministic order) and `stageRise` (per block). It is not
`spring-text-engine` because those layers do not scroll and the engine's only
externally driven mode is `manual`, which hard rule #3 forbids. Everything that
*does* scroll — the chapters, the manifesto, the collection, the lookbook
masthead — uses the engine in `mode="once"`.

**Reveal choreography** follows the house default: the curtain's counter is
asset-backed (parks at 92 until the film's first frame, floor 1600 ms, cap
7000 ms), the page is released at **half** the curtain's exit so the hero rises
through its trailing edge, above-the-fold content is mounted hidden rather than
withheld, below-the-fold reveals latch once on viewport entry, and the film
enters with a real scale-in rather than a fade.

**Consequences.** Re-skinning is a token edit in `globals.css`; restructuring
a section means picking another composition of the same role and rebuilding to
it, keeping the Style and content. A second accent, a second type system, or a
dark ground would each break the Style — see the notes in `globals.css`.
Related: [[components/common]], [[design-system]], [[text-engine]].
