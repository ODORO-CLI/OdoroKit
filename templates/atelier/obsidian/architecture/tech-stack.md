...
## 3D (the closing scene)

| Package | Version | Role |
|---------|---------|------|
| `three` | `^0.186.0` | WebGL renderer for the Onyx Cubes swarm (`src/lib/scene/onyx-cubes.ts`) — plain three.js, no R3F |
| `cannon-es` | `^0.20.0` | Rigid-body physics for the swarm: weightless bodies on a centre spring, cursor push, grab-and-fling |
| `@types/three` | `^0.186.0` | Types (dev) |

Loaded lazily — the chunk is requested only when the closing screen is a
viewport away (`components/common/scene/lazy-onyx-cubes.tsx`). The scene runs
on the shared ticker while on screen. See [[components/common]] and
[[decisions-log]] ADR-0024.

## Package manager

**npm**, and only npm: `package-lock.json` is the single lockfile and `npm ci`
is the install. The starter's `yarn.lock` was deleted — Vercel picks its
package manager from whichever lockfile it finds, and only the npm tree was
verified. See [[decisions-log]] ADR-0026 and ADR-0027.
...
```bash
npm run dev    # the dev server — local development
npm run build  # next build — production build
npm start      # next start — serve production build
npm run lint   # eslint
```

Package manager: **npm** (`package-lock.json` is committed) — ADR-0027.
