...
## 2026-09-09 (la template passe a l'agence ODORO)

- **Une capture d'ecran ne verifie PAS une scene 3D sur cette machine.** Le
  Chrome sans interface utilise pour les captures n'a **aucun contexte WebGL** :
  `canvas.getContext("webgl2")` et `getContext("webgl")` renvoient tous deux
  `null`, avec `--disable-gpu` comme avec
  `--use-gl=angle --use-angle=swiftshader --enable-unsafe-swiftshader`. La zone
  du sigle sort donc **vide** de toute capture, et cela ne dit rien sur l'etat du
  modele. Verifier autrement, et le dire :
  1. le fichier est servi (code 200, taille attendue, magie `glTF`) ;
  2. il se recharge avec le `GLTFLoader` **de la page** dans Node — nombre de
     mailles, de triangles, dimensions de l'englobant ;
...
- **Toute la copie repasse de « promoteur » a « agence ».** Le site ne vend plus
  un programme neuf : il presente **un bien a la une** pour l'agence ODORO. 53
  chaines reecrites dans les deux fichiers de contenu, plus `site.name`,
  `site.author` et un `aria-label` de la nav que le remplacement automatique
  n'atteignait pas — « AMIEL » y apparaissait **deux fois**, donc ambigu, et le
  script refuse plutot que de deviner. Le bien est **invente** et le restera :
  c'est une template, la page doit se lire comme un vrai bien sans qu'on ait a
  croire aux chiffres.
- **Le nom tombe juste sur la contrainte du wordmark.** « odoro » fait exactement
  **5 lettres**, comme « aerra » et « amiel » avant lui, donc le calage a 801 px
  tient sans rien forcer (ADR-0032).
- **Trois refus d'editeur qui valaient mieux que le brouillon.** « Le site » en
  navigation : sur un site web ca se lit d'abord comme *ce site internet*, et
  l'ambiguite est fatale dans une barre de quatre entrees — remplace par « Le
  lieu ». « Appeler » en bouton : la section visee est un **formulaire**, pas un
  numero, donc le libelle promettait une action que la page ne fournit pas.
  « 214 m² » seul : sans « habitables », la surface est juridiquement floue entre
  carrez, plancher et habitable.
- **Les titres animes ont ete mesures avant d'etre poses**, pas apres.
  `hero` : 761 px sur 810 disponibles, deux lignes.
  `contact` : 849 px puis 471 px sur 924, et le decoupage `headingLead` /
  `headingRest` fait coincider la bascule de couleur avec le retour a la ligne.
  Les cinq surtitres tiennent chacun sur une ligne, le plus juste etant
  « Acquereurs » a 104 px pour 129 disponibles.
- **`about.image.alt` dit desormais la verite** : un bonhomme baton dessine a la
  main, et non une photo de batiment. L'ancien texte decrivait un bien alors que
  le fichier livre etait un portrait en studio ; l'ecart datait de la template
  d'origine.

- **Le sigle est reconstruit, pas vectorise.** Le logo fourni est purement
  geometrique : dans une boite de 100 × 100, un coin carre en haut a gauche puis
  un arc de 270°, rayon exterieur **50** et interieur **35**, soit les **15**
  unites d'epaisseur du trait. On le decrit donc par un tracé au lieu de tracer
...
- **Le sigle 3D est genere, plus telecharge.** `public/assets/Mark/odoro-mark.glb`
  (162 Ko, 1 968 triangles) est produit a partir du meme tracé, extrude avec un
  petit biseau pour que le metal accroche une arete. `ScrollModel` applique son
  propre materiau et recentre le modele sur son englobant, donc le fichier
  n'embarque **ni materiau ni transformation** — seulement positions, normales et
  index. Verifie en le rechargeant avec le `GLTFLoader` de la page : 1 maille,
  103,2 × 103,2 × 21,2, carre. L'ancien `Who this is for/model.glb` n'est plus
  reference ; il est conserve, pas supprime.
- **L'orange de marque entre dans les trois niveaux de tokens**, comme l'impose
  [[design-system]] : `--raw-color-orange-500` (#f97316) → `--brand-mark` →
  `--color-brand-mark`. Il n'est cable **que** sur le sigle de navigation. La
  page garde son noir et blanc photographique : etendre l'orange aux boutons et
  aux liens reviendrait a poser un second Style sur la meme surface.
- **La photo d'illustration devient un dessin.** `About/about.png` etait un
  portrait d'homme en studio, sans rapport avec un bien immobilier. C'est
  desormais un bonhomme baton dessine par `tools-bonhomme-baton.py` — **21 Ko au
  lieu de 1,2 Mo**. Le trait est perturbe par deux sinusoides dephasees de longue
  periode et non par un bruit par pixel : un jitter par pixel donne un rendu
  velu, pas un rendu manuel. Chaque trait est repasse deux fois, comme on repasse
  un feutre, ce qui produit l'irregularite d'epaisseur. `random.seed(7)` rend le
  fichier reproductible. Le cercle de la tete recoit une amplitude reduite : a
  amplitude egale une courbe fermee montre l'ondulation bien plus qu'un segment
  et devient du crenelage.

## 2026-09-09 (bascule editoriale : promoteur immobilier, en francais)

- **La page ne vend plus une maison isolee, elle presente un programme neuf.**
  Toute la copie visible est passee en francais et reorientee vers un promoteur
  immobilier : AMIEL, programme « Le 24 Marquisats » a Annecy, quatorze
  logements, livraison au 4e trimestre 2028. **61 chaines** reecrites dans
  `src/data/mocks/home.ts` et `src/lib/site.ts`, aucun composant touche : le
  contrat de la template donne `mutable = skin` et `preserve = motion,
  composition`, et le contenu vit entierement dans ces deux fichiers.
- **Deux debordements trouves a la mesure, pas a l'oeil.** Le respect du budget
  en CARACTERES (ADR-0032) ne suffit pas : c'est la largeur en PIXELS qui decide.
  Les deux cas ont ete mesures avec la vraie police, a la vraie taille, contre la
  vraie largeur de boite, et non estimes.
  - `hero.headingRest` : « quatorze appartements » mesure **808 px** dans une
    boite de **810 px** (`lg:w-202.5`). Il tenait sur le papier et passait a la
    ligne a l'ecran, la troisieme ligne chevauchant le sous-titre. Remplace par
    « quatorze logements », **687 px**, soit 123 px de marge.
  - `audience.eyebrow` : « Les acquereurs » mesure **137 px** pour **129 px**
    disponibles (`lg:w-36.25` moins la pastille et le gap), ce qui laissait
    « Les » orphelin sur sa ligne. L'original anglais « Who this is for » passait
    a **128 px**, c'est-a-dire a **un pixel pres** — la ligne unique est bien
    l'intention. Remplace par « Acquereurs », 104 px.
- **`contact.headingLead` / `headingRest` redecoupes.** La coupure tombait au
  milieu de la partie claire ; le decoupage a ete deplace pour que la bascule de
  couleur coincide avec le retour a la ligne, comme dans l'original. Ligne 1
  « Venez voir le chantier, » (873 px), ligne 2 « pas une plaquette » (471 px),
  boite de 924 px.
- **Les valeurs de statistiques restent des entiers.** Le brief proposait
  « 2,90 m sous plafond ». `Number("2,90")` vaut `NaN` : `CounterValue` a un
  repli propre qui rend la valeur en texte, mais cette statistique serait restee
  figee pendant que ses deux voisines comptent et sortent du flou. Remplacee par
  « 104 / m² en moyenne ». Une virgule decimale dans `numbers.stats[].value`
  demande de modifier le composant, ce qui sort du perimetre d'un travail de
  copie.
- **Le document declare enfin sa langue.** `src/app/layout.tsx` passe de
  `<html lang="en">` a `lang="fr"` : sans cela les lecteurs d'ecran prononcent le
  francais avec une phonetique anglaise et la cesure est fausse.
- **Le reste de l'interface a suivi.** Le parcours cookies (banniere + modale de
  preferences, RGPD), les pages `error` et `not-found`, et deux `aria-label` de
  la navigation etaient encore en anglais alors que tout le contenu etait passe
  en francais. **24 chaines** traduites dans 6 fichiers. `yarn lint` a rattrape
  une apostrophe non echappee dans `not-found.tsx` (`react/no-unescaped-entities`).
- **Les identifiants de contenu ont ete realignes.** `id: "short-term-letting"`
  sur une carte intitulee « La liste d'attente » est un piege pour la prochaine
  personne qui lit le fichier. Verifie d'abord qu'aucun de ces identifiants n'est
  reference hors de `home.ts` — ce sont de simples cles React — puis renommes.
...
- **Piege d'outillage, a ne pas reproduire.** Plusieurs `next build` ont semble
  ...
  propre commande** : viser le PID qui ecoute le port (`lsof -ti tcp:3000`).
- **Un `next build` interrompu laisse un verrou.** Le suivant refuse de demarrer
  avec « Another next build process is already running », meme apres `rm -rf
...
- **Le port 3000 n'est pas forcement ce projet.** Un autre projet de la machine
  (`~/ODORO`, Next 15.5.23) ecoute sur 3000. Verifier a qui appartient le port
  (`lsof -nP -iTCP:3000 -sTCP:LISTEN`, puis le `<title>` servi) avant de lire un
  resultat ou d'arreter un processus. Lancer ce projet sur un autre port evite
  la collision.
- **Restent de vraies lectures instables**, elles sans rapport avec ce qui
  precede : le serveur de dev a renvoye a plusieurs reprises une image avec le
  bon `Content-Length` et **zero octet** de corps, et `ffmpeg` a annonce `moov
  atom not found` sur un MP4 dont `ffprobe` lit parfaitement les 97 images. Le
  fichier sur disque etait intact a chaque fois, verifie octet pour octet. Une
  simple relance suffit ; en preparant une capture, precharger les images avec
  quelques reessais avant de photographier.

## 2026-09-09 (reprise en projet neuf)

- **Le projet est reparti de l'archive complete de la template, pas fichier par
  fichier.** `getlayers_materialize` sur `aerra` avec `target: next` expose un
  `build.downloadProject` : un zip de **17,7 Mo** contenant l'arbre entier, le
  `public/` deja rempli et ce vault. Recuperer les fichiers un a un aurait donne
  le meme code, mais aurait laisse les medias a telecharger separement via des
  liens signes qui expirent en une heure. Les **12 medias** ont ete verifies
  octet pour octet contre `build.mediaAssets` : `17 451 705` octets attendus,
  autant recus, zero ecart. Ce controle n'est pas theorique — un lien expire
  ecrit une page d'erreur sous un nom en `.mp4`, et le seul symptome plus tard
  est un canvas blanc.
- **Installe avec npm, pas yarn.** Le projet livre un `yarn.lock` mais Node est
  gere ici par Volta, sans corepack : `yarn` et `corepack` sont tous deux
  absents du PATH. `npm install` sur Node **25.5.0**, puis `npm run build` :
  compilation en 10,7 s, TypeScript en 12,6 s, 9 routes generees, aucune erreur.
  Le `yarn.lock` est conserve tel quel, il reste la reference si yarn revient.
- **Aucune variable d'environnement n'est requise pour demarrer.** `src/env.ts`
  declare `NEXT_PUBLIC_SITE_URL` et `CONTACT_ENDPOINT` comme optionnelles via
  `optionalUrl()`, donc pas de `.env` a creer. Le rendu a ete confirme sur le
  serveur de dev : **156 460 octets** de HTML, les six sections presentes, les
  sept medias references.
- **Deux notes ajoutees a la racine.** `getlayers.json` enregistre l'etat de
  build GetLayers — Style engage (`aerra-style`, la template garde le sien),
  tokens resolus, les six sections placees, et les contraintes qui bornent tout
  travail editorial a venir. `MEDIAS-A-REMPLACER.md` documente les sept medias,
  leurs dimensions exactes et les trois qui se remplacent mal si on les prend
  pour de simples photos : le hero est un detourage a fond transparent pose
  devant un degrade CSS, la video de la section programme n'est jamais lue mais
  parcourue image par image (97 images = la finesse du geste), et le `mask.png`
  de la section situation doit etre cadre exactement comme la photo qu'il
  recouvre.
